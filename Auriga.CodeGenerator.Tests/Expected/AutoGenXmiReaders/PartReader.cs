...
namespace Auriga.Xmi.AutoGenXmiReaders.Cs
{
    ...
    public class PartReader : XmiElementReader<Auriga.Cs.IPart>, IXmiElementReader<Auriga.Cs.IPart>
    {
        ...
        public PartReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Cs.IPart Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "OwnedAbstractType", href); SkipElement(xmlReader); }
                            else { var contained = (Auriga.Modellingcore.IAbstractType)this.Facade.QueryElement(xmlReader); contained.Container = poco; poco.OwnedAbstractType = contained; }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Part at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
