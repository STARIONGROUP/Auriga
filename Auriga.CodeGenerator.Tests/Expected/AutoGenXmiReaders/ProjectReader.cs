...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellamodeller
{
    ...
    public class ProjectReader : XmiElementReader<Auriga.Capellamodeller.IProject>, IXmiElementReader<Auriga.Capellamodeller.IProject>
    {
        ...
        public ProjectReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellamodeller.IProject Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedFolders", href); SkipElement(xmlReader); }
                            else { poco.OwnedFolders.Add((Auriga.Capellamodeller.IFolder)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedModelRoots", href); SkipElement(xmlReader); }
                            else { poco.OwnedModelRoots.Add((Auriga.Capellamodeller.IModelRoot)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Project at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
