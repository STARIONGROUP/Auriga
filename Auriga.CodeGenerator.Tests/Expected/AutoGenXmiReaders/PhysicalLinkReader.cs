...
namespace Auriga.Xmi.AutoGenXmiReaders.Cs
{
    ...
    public class PhysicalLinkReader : XmiElementReader<Auriga.Cs.IPhysicalLink>, IXmiElementReader<Auriga.Cs.IPhysicalLink>
    {
        ...
        public PhysicalLinkReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Cs.IPhysicalLink Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "LinkEnds", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedComponentExchangeAllocations", href); SkipElement(xmlReader); }
                            else { poco.OwnedComponentExchangeAllocations.Add((Auriga.Fa.IComponentExchangeAllocation)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalLinkEnds", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalLinkEnds.Add((Auriga.Cs.IPhysicalLinkEnd)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedPhysicalLinkRealizations":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalLinkRealizations", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalLinkRealizations.Add((Auriga.Cs.IPhysicalLinkRealization)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalLink at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
