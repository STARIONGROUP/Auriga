// ------------------------------------------------------------------------------------------------
// <copyright file="StateMappingReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Sirius.Xmi.AutoGenXmiReaders.Sequence.Description
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>StateMapping</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class StateMappingReader : XmiElementReader<Auriga.Sirius.Sequence.Description.IStateMapping>, IXmiElementReader<Auriga.Sirius.Sequence.Description.IStateMapping>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="StateMappingReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public StateMappingReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>StateMapping</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Sirius.Sequence.Description.IStateMapping"/></returns>
        public Auriga.Sirius.Sequence.Description.IStateMapping Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Sirius.Sequence.Description.StateMapping();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading StateMapping at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "conditionnalStyles":
                            {
                                ...
                                {
                                    poco.ConditionnalStyles.Add((Auriga.Sirius.Diagram.Description.IConditionalNodeStyleDescription)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "style":
                            {
                                ...
                                {
                                    poco.Style = (Auriga.Sirius.Diagram.Description.Style.INodeStyleDescription)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"StateMappingReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by StateMappingReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read StateMapping but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
