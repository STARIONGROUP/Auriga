// ------------------------------------------------------------------------------------------------
// <copyright file="InteractionUseMappingReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Sirius.Xmi.AutoGenXmiReaders.Sequence.Description
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>InteractionUseMapping</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class InteractionUseMappingReader : XmiElementReader<Auriga.Sirius.Sequence.Description.IInteractionUseMapping>, IXmiElementReader<Auriga.Sirius.Sequence.Description.IInteractionUseMapping>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="InteractionUseMappingReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public InteractionUseMappingReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>InteractionUseMapping</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Sirius.Sequence.Description.IInteractionUseMapping"/></returns>
        public Auriga.Sirius.Sequence.Description.IInteractionUseMapping Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Sirius.Sequence.Description.InteractionUseMapping();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading InteractionUseMapping at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    if (TryParseEnum<Auriga.Sirius.Diagram.ContainerLayout>(xmlReader.GetAttribute("childrenPresentation"), out var parsed))
                    {
                ...
                poco.FinishingEndFinderExpression = xmlReader.GetAttribute("finishingEndFinderExpression");
                ...
                CollectMultiValueReferences(poco, "ReusedContainerMappings", xmlReader.GetAttribute("reusedContainerMappings"));
                CollectMultiValueReferences(poco, "ReusedNodeMappings", xmlReader.GetAttribute("reusedNodeMappings"));
                ...
                poco.StartingEndFinderExpression = xmlReader.GetAttribute("startingEndFinderExpression");
                {
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            case "borderedNodeMappings":
                            {
                                ...
                                {
                                    poco.BorderedNodeMappings.Add((Auriga.Sirius.Diagram.Description.INodeMapping)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "conditionnalStyles":
                            {
                                ...
                                {
                                    poco.ConditionnalStyles.Add((Auriga.Sirius.Diagram.Description.IConditionalContainerStyleDescription)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "reusedContainerMappings":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "ReusedContainerMappings", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "reusedNodeMappings":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "ReusedNodeMappings", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "style":
                            {
                                ...
                                {
                                    poco.Style = (Auriga.Sirius.Diagram.Description.Style.IContainerStyleDescription)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }

                                break;
                            }
                            case "subContainerMappings":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "SubContainerMappings", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.SubContainerMappings.Add((Auriga.Sirius.Diagram.Description.IContainerMapping)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }

                                break;
                            }
                            case "subNodeMappings":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "SubNodeMappings", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.SubNodeMappings.Add((Auriga.Sirius.Diagram.Description.INodeMapping)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"InteractionUseMappingReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by InteractionUseMappingReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read InteractionUseMapping but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
