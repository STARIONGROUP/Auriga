// ------------------------------------------------------------------------------------------------
// <copyright file="CombinedFragmentMappingReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Sirius.Xmi.AutoGenXmiReaders.Sequence.Description
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>CombinedFragmentMapping</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class CombinedFragmentMappingReader : XmiElementReader<Auriga.Sirius.Sequence.Description.ICombinedFragmentMapping>, IXmiElementReader<Auriga.Sirius.Sequence.Description.ICombinedFragmentMapping>
    {
        /// <summary>
        ...
        public CombinedFragmentMappingReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Sirius.Sequence.Description.ICombinedFragmentMapping"/></returns>
        public Auriga.Sirius.Sequence.Description.ICombinedFragmentMapping Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Sirius.Sequence.Description.CombinedFragmentMapping();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading CombinedFragmentMapping at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
            ...
            {
                this.Logger.LogWarning("Expected an element to read CombinedFragmentMapping but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
