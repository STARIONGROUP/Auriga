// ------------------------------------------------------------------------------------------------
// <copyright file="IntegerLayoutOptionReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Sirius.Xmi.AutoGenXmiReaders.Diagram.Description
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>IntegerLayoutOption</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class IntegerLayoutOptionReader : XmiElementReader<Auriga.Sirius.Diagram.Description.IIntegerLayoutOption>, IXmiElementReader<Auriga.Sirius.Diagram.Description.IIntegerLayoutOption>
    {
        /// <summary>
        ...
        public IntegerLayoutOptionReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>IntegerLayoutOption</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Sirius.Diagram.Description.IIntegerLayoutOption"/></returns>
        public Auriga.Sirius.Diagram.Description.IIntegerLayoutOption Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Sirius.Diagram.Description.IntegerLayoutOption();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading IntegerLayoutOption at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"IntegerLayoutOptionReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by IntegerLayoutOptionReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            ...
            {
                this.Logger.LogWarning("Expected an element to read IntegerLayoutOption but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
