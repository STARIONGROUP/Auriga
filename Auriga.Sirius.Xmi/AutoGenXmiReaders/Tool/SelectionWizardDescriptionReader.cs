// ------------------------------------------------------------------------------------------------
// <copyright file="SelectionWizardDescriptionReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Sirius.Xmi.AutoGenXmiReaders.Viewpoint.Description.Tool
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>SelectionWizardDescription</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class SelectionWizardDescriptionReader : XmiElementReader<Auriga.Sirius.Viewpoint.Description.Tool.ISelectionWizardDescription>, IXmiElementReader<Auriga.Sirius.Viewpoint.Description.Tool.ISelectionWizardDescription>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="SelectionWizardDescriptionReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public SelectionWizardDescriptionReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Sirius.Viewpoint.Description.Tool.ISelectionWizardDescription"/></returns>
        public Auriga.Sirius.Viewpoint.Description.Tool.ISelectionWizardDescription Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Sirius.Viewpoint.Description.Tool.SelectionWizardDescription();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading SelectionWizardDescription at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
            ...
            {
                this.Logger.LogWarning("Expected an element to read SelectionWizardDescription but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
