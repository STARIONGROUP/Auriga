// ------------------------------------------------------------------------------------------------
// <copyright file="TReturnMessageMappingReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Sirius.Xmi.AutoGenXmiReaders.Sequence.Template
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>TReturnMessageMapping</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class TReturnMessageMappingReader : XmiElementReader<Auriga.Sirius.Sequence.Template.ITReturnMessageMapping>, IXmiElementReader<Auriga.Sirius.Sequence.Template.ITReturnMessageMapping>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="TReturnMessageMappingReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public TReturnMessageMappingReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Sirius.Sequence.Template.ITReturnMessageMapping"/></returns>
        public Auriga.Sirius.Sequence.Template.ITReturnMessageMapping Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Sirius.Sequence.Template.TReturnMessageMapping();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading TReturnMessageMapping at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectSingleValueReference(poco, "InvocationMapping", xmlReader.GetAttribute("invocationMapping"));
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "invocationMapping":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "InvocationMapping", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
            ...
            {
                this.Logger.LogWarning("Expected an element to read TReturnMessageMapping but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
