...
namespace Auriga.Sirius.Xmi.AutoGenXmiReaders.Notation
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>Connector</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class ConnectorReader : XmiElementReader<Auriga.Sirius.Notation.IConnector>, IXmiElementReader<Auriga.Sirius.Notation.IConnector>
    {
        /// <summary>
        ...
        public ConnectorReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>Connector</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Sirius.Notation.IConnector"/></returns>
        public Auriga.Sirius.Notation.IConnector Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Sirius.Notation.Connector();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    var raw = xmlReader.GetAttribute("avoidObstructions");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.AvoidObstructions = parsed;
                    }
                }
                {
                    var raw = xmlReader.GetAttribute("closestDistance");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.ClosestDistance = parsed;
                    }
                }
                CollectSingleValueReference(poco, "Element", xmlReader.GetAttribute("element"));
                {
                    if (TryParseEnum<Auriga.Sirius.Notation.JumpLinkStatus>(xmlReader.GetAttribute("jumpLinkStatus"), out var parsed))
                    {
                        poco.JumpLinkStatus = parsed;
                    }
                }
                {
                    if (TryParseEnum<Auriga.Sirius.Notation.JumpLinkType>(xmlReader.GetAttribute("jumpLinkType"), out var parsed))
                    {
                        poco.JumpLinkType = parsed;
                    }
                }
                {
                    var raw = xmlReader.GetAttribute("jumpLinksReverse");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.JumpLinksReverse = parsed;
                    }
                }
                {
                ...
                {
                    if (TryParseEnum<Auriga.Sirius.Notation.Routing>(xmlReader.GetAttribute("routing"), out var parsed))
                    {
                        poco.Routing = parsed;
                    }
                }
                {
                    if (TryParseEnum<Auriga.Sirius.Notation.Smoothness>(xmlReader.GetAttribute("smoothness"), out var parsed))
                    {
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            case "bendpoints":
                            {
                                ...
                                {
                                    CollectSingleValueReference(poco, "Bendpoints", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.Bendpoints = (Auriga.Sirius.Notation.IBendpoints)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "sourceAnchor":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "SourceAnchor", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.SourceAnchor = (Auriga.Sirius.Notation.IAnchor)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "targetAnchor":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "TargetAnchor", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.TargetAnchor = (Auriga.Sirius.Notation.IAnchor)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                this.Logger.LogWarning("Not supported by ConnectorReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read Connector but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
