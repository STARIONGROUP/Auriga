...
namespace Auriga.Xmi.AutoGenXmiReaders.Pa
{
    ...
    public class PhysicalComponentPkgReader : XmiElementReader<Auriga.Pa.IPhysicalComponentPkg>, IXmiElementReader<Auriga.Pa.IPhysicalComponentPkg>
    {
        ...
        public PhysicalComponentPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Pa.IPhysicalComponentPkg Read(XmlReader xmlReader)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedAssociations", href); SkipElement(xmlReader); }
                            else { poco.OwnedAssociations.Add((Auriga.Information.IAssociation)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedDeployments", href); SkipElement(xmlReader); }
                            else { poco.OwnedDeployments.Add((Auriga.Cs.IAbstractDeploymentLink)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedKeyParts", href); SkipElement(xmlReader); }
                            else { poco.OwnedKeyParts.Add((Auriga.Information.IKeyPart)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalComponentPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            ...
            {
                this.Logger.LogWarning("Expected an element to read PhysicalComponentPkg but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
