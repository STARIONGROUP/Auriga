...
namespace Auriga.Xmi.AutoGenXmiReaders.Pa
{
    ...
    public class PhysicalFunctionPkgReader : XmiElementReader<Auriga.Pa.IPhysicalFunctionPkg>, IXmiElementReader<Auriga.Pa.IPhysicalFunctionPkg>
    {
        ...
        public PhysicalFunctionPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Pa.IPhysicalFunctionPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedCategories", href); SkipElement(xmlReader); }
                            else { poco.OwnedCategories.Add((Auriga.Fa.IExchangeCategory)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedExchangeSpecificationRealizations", href); SkipElement(xmlReader); }
                            else { poco.OwnedExchangeSpecificationRealizations.Add((Auriga.Fa.IExchangeSpecificationRealization)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedExchanges":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedExchanges", href); SkipElement(xmlReader); }
                            else { poco.OwnedExchanges.Add((Auriga.Fa.IFunctionalExchangeSpecification)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedFunctionSpecifications", href); SkipElement(xmlReader); }
                            else { poco.OwnedFunctionSpecifications.Add((Auriga.Fa.IFunctionSpecification)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalFunctions", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalFunctions.Add((Auriga.Pa.IPhysicalFunction)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalFunctionPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
