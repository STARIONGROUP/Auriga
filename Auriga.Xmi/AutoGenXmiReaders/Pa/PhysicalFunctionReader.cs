// ------------------------------------------------------------------------------------------------
// <copyright file="PhysicalFunctionReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.AutoGenXmiReaders.Pa
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>PhysicalFunction</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class PhysicalFunctionReader : XmiElementReader<Auriga.Pa.IPhysicalFunction>, IXmiElementReader<Auriga.Pa.IPhysicalFunction>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="PhysicalFunctionReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public PhysicalFunctionReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
            : base(cache, facade, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>PhysicalFunction</c> from the element at the cursor of the supplied reader.
        /// </summary>
        /// <param name="xmlReader">the reader positioned on the element</param>
        /// <returns>the populated <see cref="Auriga.Pa.IPhysicalFunction"/></returns>
        public Auriga.Pa.IPhysicalFunction Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Pa.PhysicalFunction();
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "ownedNullValue":
                        {
                            ...
                            case "ownedPhysicalFunctionPkgs":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalFunctionPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalFunctionPkgs.Add((Auriga.Pa.IPhysicalFunctionPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalFunction at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            ...
            {
                this.Logger.LogWarning("Expected an element to read PhysicalFunction but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
