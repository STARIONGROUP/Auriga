...
namespace Auriga.Xmi.AutoGenXmiReaders.Pa
{
    ...
    public class PhysicalComponentReader : XmiElementReader<Auriga.Pa.IPhysicalComponent>, IXmiElementReader<Auriga.Pa.IPhysicalComponent>
    {
        ...
        public Auriga.Pa.IPhysicalComponent Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedDeploymentLinks", href); SkipElement(xmlReader); }
                            else { poco.OwnedDeploymentLinks.Add((Auriga.Cs.IAbstractDeploymentLink)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalComponentPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalComponentPkgs.Add((Auriga.Pa.IPhysicalComponentPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedPhysicalComponents":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalComponents", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalComponents.Add((Auriga.Pa.IPhysicalComponent)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalComponent at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
