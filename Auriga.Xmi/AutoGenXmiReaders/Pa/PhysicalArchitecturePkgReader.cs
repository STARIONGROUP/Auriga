...
namespace Auriga.Xmi.AutoGenXmiReaders.Pa
{
    ...
    public class PhysicalArchitecturePkgReader : XmiElementReader<Auriga.Pa.IPhysicalArchitecturePkg>, IXmiElementReader<Auriga.Pa.IPhysicalArchitecturePkg>
    {
        ...
        public PhysicalArchitecturePkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Pa.IPhysicalArchitecturePkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalArchitecturePkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalArchitecturePkgs.Add((Auriga.Pa.IPhysicalArchitecturePkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedPhysicalArchitectures":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalArchitectures", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalArchitectures.Add((Auriga.Pa.IPhysicalArchitecture)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalArchitecturePkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
