...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Datatype
{
    ...
    public class PhysicalQuantityReader : XmiElementReader<Auriga.Information.Datatype.IPhysicalQuantity>, IXmiElementReader<Auriga.Information.Datatype.IPhysicalQuantity>
    {
        ...
        public Auriga.Information.Datatype.IPhysicalQuantity Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalQuantity at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
