// ------------------------------------------------------------------------------------------------
// <copyright file="PhysicalQuantityReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Datatype
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>PhysicalQuantity</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class PhysicalQuantityReader : XmiElementReader<Auriga.Information.Datatype.IPhysicalQuantity>, IXmiElementReader<Auriga.Information.Datatype.IPhysicalQuantity>
    {
        /// <summary>
        ...
        public PhysicalQuantityReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>PhysicalQuantity</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Information.Datatype.IPhysicalQuantity"/></returns>
        public Auriga.Information.Datatype.IPhysicalQuantity Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Information.Datatype.PhysicalQuantity();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading PhysicalQuantity at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    var raw = xmlReader.GetAttribute("discrete");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.Discrete = parsed;
                    }
                }
                ...
                {
                    if (TryParseEnum<Auriga.Information.Datatype.NumericTypeKind>(xmlReader.GetAttribute("kind"), out var parsed))
                    {
                        poco.Kind = parsed;
                    }
                }
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedDataValues":
                            {
                                ...
                                {
                                    poco.OwnedDataValues.Add((Auriga.Information.Datavalue.IDataValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedDefaultValue":
                            {
                                ...
                                {
                                    poco.OwnedDefaultValue = (Auriga.Information.Datavalue.INumericValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "ownedInformationRealizations":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedInformationRealizations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedInformationRealizations.Add((Auriga.Information.IInformationRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedMaxValue":
                            {
                                ...
                                {
                                    poco.OwnedMaxValue = (Auriga.Information.Datavalue.INumericValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "ownedMinValue":
                            {
                                ...
                                {
                                    poco.OwnedMinValue = (Auriga.Information.Datavalue.INumericValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "ownedNullValue":
                            {
                                ...
                                {
                                    poco.OwnedNullValue = (Auriga.Information.Datavalue.INumericValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"PhysicalQuantityReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by PhysicalQuantityReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read PhysicalQuantity but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
