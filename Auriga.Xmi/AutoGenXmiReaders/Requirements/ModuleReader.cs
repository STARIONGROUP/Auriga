...
namespace Auriga.Xmi.AutoGenXmiReaders.Requirements
{
    ...
    public class ModuleReader : XmiElementReader<Auriga.Requirements.IModule>, IXmiElementReader<Auriga.Requirements.IModule>
    {
        ...
        public ModuleReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "ownedRequirements":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Module at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
