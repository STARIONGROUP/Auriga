...
namespace Auriga.Xmi.AutoGenXmiReaders.Requirements
{
    ...
    public class DataTypeDefinitionReader : XmiElementReader<Auriga.Requirements.IDataTypeDefinition>, IXmiElementReader<Auriga.Requirements.IDataTypeDefinition>
    {
        ...
        public DataTypeDefinitionReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "ownedExtensions":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of DataTypeDefinition at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
