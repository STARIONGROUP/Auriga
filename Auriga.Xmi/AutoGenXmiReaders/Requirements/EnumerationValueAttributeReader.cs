...
namespace Auriga.Xmi.AutoGenXmiReaders.Requirements
{
    ...
    public class EnumerationValueAttributeReader : XmiElementReader<Auriga.Requirements.IEnumerationValueAttribute>, IXmiElementReader<Auriga.Requirements.IEnumerationValueAttribute>
    {
        ...
        public EnumerationValueAttributeReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "Values", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of EnumerationValueAttribute at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
