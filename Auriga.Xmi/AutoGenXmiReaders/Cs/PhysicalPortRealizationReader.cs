...
namespace Auriga.Xmi.AutoGenXmiReaders.Cs
{
    ...
    public class PhysicalPortRealizationReader : XmiElementReader<Auriga.Cs.IPhysicalPortRealization>, IXmiElementReader<Auriga.Cs.IPhysicalPortRealization>
    {
        ...
        public PhysicalPortRealizationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Cs.IPhysicalPortRealization Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalPortRealization at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
