...
namespace Auriga.Xmi.AutoGenXmiReaders.Cs
{
    ...
    public class PhysicalPathInvolvementReader : XmiElementReader<Auriga.Cs.IPhysicalPathInvolvement>, IXmiElementReader<Auriga.Cs.IPhysicalPathInvolvement>
    {
        ...
        public PhysicalPathInvolvementReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Cs.IPhysicalPathInvolvement Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "NextInvolvements", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalPathInvolvement at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
