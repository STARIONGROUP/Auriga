...
namespace Auriga.Xmi.AutoGenXmiReaders.Cs
{
    ...
    public class InterfaceReader : XmiElementReader<Auriga.Cs.IInterface>, IXmiElementReader<Auriga.Cs.IInterface>
    {
        ...
        public InterfaceReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedExchangeItemAllocations", href); SkipElement(xmlReader); }
                            else { poco.OwnedExchangeItemAllocations.Add((Auriga.Cs.IExchangeItemAllocation)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
