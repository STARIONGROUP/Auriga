...
namespace Auriga.Xmi.AutoGenXmiReaders.Cs
{
    ...
    public class InterfacePkgReader : XmiElementReader<Auriga.Cs.IInterfacePkg>, IXmiElementReader<Auriga.Cs.IInterfacePkg>
    {
        ...
        public InterfacePkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Cs.IInterfacePkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedInterfacePkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedInterfacePkgs.Add((Auriga.Cs.IInterfacePkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedInterfaces":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedInterfaces", href); SkipElement(xmlReader); }
                            else { poco.OwnedInterfaces.Add((Auriga.Cs.IInterface)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of InterfacePkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
