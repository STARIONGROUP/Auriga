// ------------------------------------------------------------------------------------------------
// <copyright file="ExchangeItemAllocationReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.AutoGenXmiReaders.Cs
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>ExchangeItemAllocation</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class ExchangeItemAllocationReader : XmiElementReader<Auriga.Cs.IExchangeItemAllocation>, IXmiElementReader<Auriga.Cs.IExchangeItemAllocation>
    {
        /// <summary>
        ...
        public ExchangeItemAllocationReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>ExchangeItemAllocation</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Cs.IExchangeItemAllocation"/></returns>
        public Auriga.Cs.IExchangeItemAllocation Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Cs.ExchangeItemAllocation();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading ExchangeItemAllocation at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectSingleValueReference(poco, "AllocatedItem", xmlReader.GetAttribute("allocatedItem"));
                ...
                {
                    if (TryParseEnum<Auriga.Information.Communication.CommunicationLinkProtocol>(xmlReader.GetAttribute("receiveProtocol"), out var parsed))
                    {
                ...
                {
                    if (TryParseEnum<Auriga.Information.Communication.CommunicationLinkProtocol>(xmlReader.GetAttribute("sendProtocol"), out var parsed))
                    {
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            case "allocatedItem":
                            {
                                ...
                                {
                                    CollectSingleValueReference(poco, "AllocatedItem", href);
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ExchangeItemAllocationReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ExchangeItemAllocationReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read ExchangeItemAllocation but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
