...
namespace Auriga.Xmi.AutoGenXmiReaders.Cs
{
    ...
    public class ExchangeItemAllocationReader : XmiElementReader<Auriga.Cs.IExchangeItemAllocation>, IXmiElementReader<Auriga.Cs.IExchangeItemAllocation>
    {
        ...
        public ExchangeItemAllocationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Cs.IExchangeItemAllocation Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { if (TryParseEnum<Auriga.Information.Communication.CommunicationLinkProtocol>(xmlReader.GetAttribute("receiveProtocol"), out var parsed)) { poco.ReceiveProtocol = parsed; } }
                poco.Review = xmlReader.GetAttribute("review");
                { if (TryParseEnum<Auriga.Information.Communication.CommunicationLinkProtocol>(xmlReader.GetAttribute("sendProtocol"), out var parsed)) { poco.SendProtocol = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "allocatedItem":
                        {
                            ...
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "AllocatedItem", href); SkipElement(xmlReader); }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ExchangeItemAllocation at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
