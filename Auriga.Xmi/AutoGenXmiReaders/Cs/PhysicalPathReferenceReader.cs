...
namespace Auriga.Xmi.AutoGenXmiReaders.Cs
{
    ...
    public class PhysicalPathReferenceReader : XmiElementReader<Auriga.Cs.IPhysicalPathReference>, IXmiElementReader<Auriga.Cs.IPhysicalPathReference>
    {
        ...
        public PhysicalPathReferenceReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Cs.IPhysicalPathReference Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalPathReference at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
