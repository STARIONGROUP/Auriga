// ------------------------------------------------------------------------------------------------
// <copyright file="FlatContainerStyleDescriptionReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Diagram.AutoGenXmiReaders.Diagram.Description.Style
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>FlatContainerStyleDescription</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class FlatContainerStyleDescriptionReader : XmiElementReader<Auriga.Diagram.Diagram.Description.Style.IFlatContainerStyleDescription>, IXmiElementReader<Auriga.Diagram.Diagram.Description.Style.IFlatContainerStyleDescription>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="FlatContainerStyleDescriptionReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public FlatContainerStyleDescriptionReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>FlatContainerStyleDescription</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Diagram.Diagram.Description.Style.IFlatContainerStyleDescription"/></returns>
        public Auriga.Diagram.Diagram.Description.Style.IFlatContainerStyleDescription Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Diagram.Diagram.Description.Style.FlatContainerStyleDescription();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading FlatContainerStyleDescription at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    var raw = xmlReader.GetAttribute("arcHeight");
                    ...
                    {
                        poco.ArcHeight = parsed;
                    }
                }
                {
                    var raw = xmlReader.GetAttribute("arcWidth");
                    if (!string.IsNullOrEmpty(raw) && int.TryParse(raw, System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out var parsed))
                    {
                        poco.ArcWidth = parsed;
                    }
                }
                CollectSingleValueReference(poco, "BackgroundColor", xmlReader.GetAttribute("backgroundColor"));
                {
                    if (TryParseEnum<Auriga.Diagram.Diagram.BackgroundStyle>(xmlReader.GetAttribute("backgroundStyle"), out var parsed))
                    {
                        poco.BackgroundStyle = parsed;
                    }
                }
                ...
                CollectSingleValueReference(poco, "ForegroundColor", xmlReader.GetAttribute("foregroundColor"));
                poco.HeightComputationExpression = xmlReader.GetAttribute("heightComputationExpression");
                {
                ...
                {
                    if (TryParseEnum<Auriga.Diagram.Viewpoint.LabelAlignment>(xmlReader.GetAttribute("labelAlignment"), out var parsed))
                    {
                        poco.LabelAlignment = parsed;
                    }
                }
                CollectSingleValueReference(poco, "LabelBorderStyle", xmlReader.GetAttribute("labelBorderStyle"));
                ...
                {
                    var raw = xmlReader.GetAttribute("roundedCorner");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.RoundedCorner = parsed;
                    }
                }
                {
                ...
                poco.WidthComputationExpression = xmlReader.GetAttribute("widthComputationExpression");
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "foregroundColor":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "ForegroundColor", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "labelBorderStyle":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "LabelBorderStyle", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"FlatContainerStyleDescriptionReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by FlatContainerStyleDescriptionReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read FlatContainerStyleDescription but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
