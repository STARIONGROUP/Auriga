// ------------------------------------------------------------------------------------------------
// <copyright file="ReturnMessageMappingReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Diagram.AutoGenXmiReaders.Sequence.Description
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>ReturnMessageMapping</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class ReturnMessageMappingReader : XmiElementReader<Auriga.Diagram.Sequence.Description.IReturnMessageMapping>, IXmiElementReader<Auriga.Diagram.Sequence.Description.IReturnMessageMapping>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ReturnMessageMappingReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public ReturnMessageMappingReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>ReturnMessageMapping</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Diagram.Sequence.Description.IReturnMessageMapping"/></returns>
        public Auriga.Diagram.Sequence.Description.IReturnMessageMapping Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Diagram.Sequence.Description.ReturnMessageMapping();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading ReturnMessageMapping at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                poco.InvocationMessageFinderExpression = xmlReader.GetAttribute("invocationMessageFinderExpression");
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            case "conditionnalStyles":
                            {
                                ...
                                {
                                    poco.ConditionnalStyles.Add((Auriga.Diagram.Diagram.Description.IConditionalEdgeStyleDescription)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "style":
                            {
                                ...
                                {
                                    poco.Style = (Auriga.Diagram.Diagram.Description.Style.IEdgeStyleDescription)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ReturnMessageMappingReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ReturnMessageMappingReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read ReturnMessageMapping but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
