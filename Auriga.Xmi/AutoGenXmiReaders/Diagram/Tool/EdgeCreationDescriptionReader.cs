// ------------------------------------------------------------------------------------------------
// <copyright file="EdgeCreationDescriptionReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Diagram.AutoGenXmiReaders.Diagram.Description.Tool
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>EdgeCreationDescription</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class EdgeCreationDescriptionReader : XmiElementReader<Auriga.Diagram.Diagram.Description.Tool.IEdgeCreationDescription>, IXmiElementReader<Auriga.Diagram.Diagram.Description.Tool.IEdgeCreationDescription>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="EdgeCreationDescriptionReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public EdgeCreationDescriptionReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>EdgeCreationDescription</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Diagram.Diagram.Description.Tool.IEdgeCreationDescription"/></returns>
        public Auriga.Diagram.Diagram.Description.Tool.IEdgeCreationDescription Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Diagram.Diagram.Description.Tool.EdgeCreationDescription();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading EdgeCreationDescription at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "initialOperation":
                            {
                                ...
                                {
                                    poco.InitialOperation = (Auriga.Diagram.Viewpoint.Description.Tool.IInitEdgeCreationOperation)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "sourceVariable":
                            {
                                ...
                                {
                                    poco.SourceVariable = (Auriga.Diagram.Diagram.Description.Tool.ISourceEdgeCreationVariable)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "sourceViewVariable":
                            {
                                ...
                                {
                                    poco.SourceViewVariable = (Auriga.Diagram.Diagram.Description.Tool.ISourceEdgeViewCreationVariable)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "targetVariable":
                            {
                                ...
                                {
                                    poco.TargetVariable = (Auriga.Diagram.Diagram.Description.Tool.ITargetEdgeCreationVariable)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "targetViewVariable":
                            {
                                ...
                                {
                                    poco.TargetViewVariable = (Auriga.Diagram.Diagram.Description.Tool.ITargetEdgeViewCreationVariable)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"EdgeCreationDescriptionReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by EdgeCreationDescriptionReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read EdgeCreationDescription but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
