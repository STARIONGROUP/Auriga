// ------------------------------------------------------------------------------------------------
// <copyright file="CombinedFragmentCreationToolReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Diagram.AutoGenXmiReaders.Sequence.Description.Tool
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>CombinedFragmentCreationTool</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class CombinedFragmentCreationToolReader : XmiElementReader<Auriga.Diagram.Sequence.Description.Tool.ICombinedFragmentCreationTool>, IXmiElementReader<Auriga.Diagram.Sequence.Description.Tool.ICombinedFragmentCreationTool>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="CombinedFragmentCreationToolReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public CombinedFragmentCreationToolReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>CombinedFragmentCreationTool</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Diagram.Sequence.Description.Tool.ICombinedFragmentCreationTool"/></returns>
        public Auriga.Diagram.Sequence.Description.Tool.ICombinedFragmentCreationTool Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Diagram.Sequence.Description.Tool.CombinedFragmentCreationTool();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading CombinedFragmentCreationTool at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "coveredLifelines":
                            {
                                ...
                                {
                                    poco.CoveredLifelines = (Auriga.Diagram.Sequence.Description.ICoveredLifelinesVariable)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "finishingEndPredecessor":
                            {
                                ...
                                {
                                    poco.FinishingEndPredecessor = (Auriga.Diagram.Sequence.Description.IMessageEndVariable)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "initialOperation":
                            {
                                ...
                                {
                                    poco.InitialOperation = (Auriga.Diagram.Viewpoint.Description.Tool.IInitialNodeCreationOperation)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "startingEndPredecessor":
                            {
                                ...
                                {
                                    poco.StartingEndPredecessor = (Auriga.Diagram.Sequence.Description.IMessageEndVariable)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "variable":
                            {
                                ...
                                {
                                    poco.Variable = (Auriga.Diagram.Diagram.Description.Tool.INodeCreationVariable)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "viewVariable":
                            {
                                ...
                                {
                                    poco.ViewVariable = (Auriga.Diagram.Viewpoint.Description.Tool.IContainerViewVariable)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CombinedFragmentCreationToolReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by CombinedFragmentCreationToolReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read CombinedFragmentCreationTool but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
