// ------------------------------------------------------------------------------------------------
// <copyright file="BracketEdgeStyleReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Diagram.AutoGenXmiReaders.Diagram
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>BracketEdgeStyle</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class BracketEdgeStyleReader : XmiElementReader<Auriga.Diagram.Diagram.IBracketEdgeStyle>, IXmiElementReader<Auriga.Diagram.Diagram.IBracketEdgeStyle>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="BracketEdgeStyleReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public BracketEdgeStyleReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>BracketEdgeStyle</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Diagram.Diagram.IBracketEdgeStyle"/></returns>
        public Auriga.Diagram.Diagram.IBracketEdgeStyle Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Diagram.Diagram.BracketEdgeStyle();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading BracketEdgeStyle at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            case "beginLabelStyle":
                            {
                                ...
                                {
                                    poco.BeginLabelStyle = (Auriga.Diagram.Diagram.IBeginLabelStyle)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "centerLabelStyle":
                            {
                                ...
                                {
                                    poco.CenterLabelStyle = (Auriga.Diagram.Diagram.ICenterLabelStyle)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "endLabelStyle":
                            {
                                ...
                                {
                                    poco.EndLabelStyle = (Auriga.Diagram.Diagram.IEndLabelStyle)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"BracketEdgeStyleReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by BracketEdgeStyleReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read BracketEdgeStyle but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
