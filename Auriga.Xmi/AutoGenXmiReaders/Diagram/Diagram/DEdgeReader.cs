// ------------------------------------------------------------------------------------------------
// <copyright file="DEdgeReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Diagram.AutoGenXmiReaders.Diagram
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>DEdge</c> from its
    ...
    public class DEdgeReader : XmiElementReader<Auriga.Diagram.Diagram.IDEdge>, IXmiElementReader<Auriga.Diagram.Diagram.IDEdge>
    {
        ...
        public DEdgeReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>DEdge</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Diagram.Diagram.IDEdge"/></returns>
        public Auriga.Diagram.Diagram.IDEdge Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Diagram.Diagram.DEdge();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading DEdge at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    var raw = xmlReader.GetAttribute("isMockEdge");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.IsMockEdge = parsed;
                    }
                }
                ...
                CollectSingleValueReference(poco, "SourceNode", xmlReader.GetAttribute("sourceNode"));
                CollectSingleValueReference(poco, "Target", xmlReader.GetAttribute("target"));
                CollectSingleValueReference(poco, "TargetNode", xmlReader.GetAttribute("targetNode"));
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedStyle":
                            {
                                ...
                                {
                                    poco.OwnedStyle = (Auriga.Diagram.Diagram.IEdgeStyle)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "path":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "Path", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "sourceNode":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "SourceNode", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "targetNode":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "TargetNode", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"DEdgeReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by DEdgeReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read DEdge but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
