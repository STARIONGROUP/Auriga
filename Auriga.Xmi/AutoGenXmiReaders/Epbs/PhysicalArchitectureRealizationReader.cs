...
namespace Auriga.Xmi.AutoGenXmiReaders.Epbs
{
    ...
    public class PhysicalArchitectureRealizationReader : XmiElementReader<Auriga.Epbs.IPhysicalArchitectureRealization>, IXmiElementReader<Auriga.Epbs.IPhysicalArchitectureRealization>
    {
        ...
        public PhysicalArchitectureRealizationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Epbs.IPhysicalArchitectureRealization Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalArchitectureRealization at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
