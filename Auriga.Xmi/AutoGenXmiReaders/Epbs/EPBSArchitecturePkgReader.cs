...
namespace Auriga.Xmi.AutoGenXmiReaders.Epbs
{
    ...
    public class EPBSArchitecturePkgReader : XmiElementReader<Auriga.Epbs.IEPBSArchitecturePkg>, IXmiElementReader<Auriga.Epbs.IEPBSArchitecturePkg>
    {
        ...
        public EPBSArchitecturePkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Epbs.IEPBSArchitecturePkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedEPBSArchitectures", href); SkipElement(xmlReader); }
                            else { poco.OwnedEPBSArchitectures.Add((Auriga.Epbs.IEPBSArchitecture)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of EPBSArchitecturePkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
