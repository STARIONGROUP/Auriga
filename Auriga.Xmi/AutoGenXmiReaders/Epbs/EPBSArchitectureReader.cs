...
namespace Auriga.Xmi.AutoGenXmiReaders.Epbs
{
    ...
    public class EPBSArchitectureReader : XmiElementReader<Auriga.Epbs.IEPBSArchitecture>, IXmiElementReader<Auriga.Epbs.IEPBSArchitecture>
    {
        ...
        public Auriga.Epbs.IEPBSArchitecture Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "OwnedConfigurationItemPkg", href); SkipElement(xmlReader); }
                            else { var contained = (Auriga.Epbs.IConfigurationItemPkg)this.Facade.QueryElement(xmlReader); contained.Container = poco; poco.OwnedConfigurationItemPkg = contained; }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalArchitectureRealizations", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalArchitectureRealizations.Add((Auriga.Epbs.IPhysicalArchitectureRealization)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of EPBSArchitecture at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
