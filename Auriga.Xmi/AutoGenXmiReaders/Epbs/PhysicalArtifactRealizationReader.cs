...
namespace Auriga.Xmi.AutoGenXmiReaders.Epbs
{
    ...
    public class PhysicalArtifactRealizationReader : XmiElementReader<Auriga.Epbs.IPhysicalArtifactRealization>, IXmiElementReader<Auriga.Epbs.IPhysicalArtifactRealization>
    {
        ...
        public PhysicalArtifactRealizationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Epbs.IPhysicalArtifactRealization Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PhysicalArtifactRealization at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
