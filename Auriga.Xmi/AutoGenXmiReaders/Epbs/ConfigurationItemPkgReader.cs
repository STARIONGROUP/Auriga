...
namespace Auriga.Xmi.AutoGenXmiReaders.Epbs
{
    ...
    public class ConfigurationItemPkgReader : XmiElementReader<Auriga.Epbs.IConfigurationItemPkg>, IXmiElementReader<Auriga.Epbs.IConfigurationItemPkg>
    {
        ...
        public ConfigurationItemPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Epbs.IConfigurationItemPkg Read(XmlReader xmlReader)
        {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ConfigurationItemPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            ...
            {
                this.Logger.LogWarning("Expected an element to read ConfigurationItemPkg but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
