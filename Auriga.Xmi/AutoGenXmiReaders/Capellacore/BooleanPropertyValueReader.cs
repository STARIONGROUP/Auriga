...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacore
{
    ...
    public class BooleanPropertyValueReader : XmiElementReader<Auriga.Capellacore.IBooleanPropertyValue>, IXmiElementReader<Auriga.Capellacore.IBooleanPropertyValue>
    {
        ...
        public BooleanPropertyValueReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacore.IBooleanPropertyValue Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { var raw = xmlReader.GetAttribute("value"); if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed)) { poco.Value = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of BooleanPropertyValue at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
