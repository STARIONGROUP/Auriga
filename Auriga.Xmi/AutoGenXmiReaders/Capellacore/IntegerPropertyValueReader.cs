...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacore
{
    ...
    public class IntegerPropertyValueReader : XmiElementReader<Auriga.Capellacore.IIntegerPropertyValue>, IXmiElementReader<Auriga.Capellacore.IIntegerPropertyValue>
    {
        ...
        public IntegerPropertyValueReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacore.IIntegerPropertyValue Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { var raw = xmlReader.GetAttribute("value"); if (!string.IsNullOrEmpty(raw) && int.TryParse(raw, System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.InvariantCulture, out var parsed)) { poco.Value = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of IntegerPropertyValue at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
