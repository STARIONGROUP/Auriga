...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacore
{
    ...
    public class ConstraintReader : XmiElementReader<Auriga.Capellacore.IConstraint>, IXmiElementReader<Auriga.Capellacore.IConstraint>
    {
        ...
        public ConstraintReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacore.IConstraint Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "ConstrainedElements", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "features":
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "OwnedSpecification", href); SkipElement(xmlReader); }
                            else { var contained = (Auriga.Modellingcore.IValueSpecification)this.Facade.QueryElement(xmlReader); contained.Container = poco; poco.OwnedSpecification = contained; }
                            break;
                        }
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Constraint at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
