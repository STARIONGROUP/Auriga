...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacore
{
    ...
    public class KeyValueReader : XmiElementReader<Auriga.Capellacore.IKeyValue>, IXmiElementReader<Auriga.Capellacore.IKeyValue>
    {
        ...
        public Auriga.Capellacore.IKeyValue Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of KeyValue at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
