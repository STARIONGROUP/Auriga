...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacore
{
    ...
    public class NamingRuleReader : XmiElementReader<Auriga.Capellacore.INamingRule>, IXmiElementReader<Auriga.Capellacore.INamingRule>
    {
        ...
        public Auriga.Capellacore.INamingRule Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        break;
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of NamingRule at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
