...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacore
{
    ...
    public class StringPropertyValueReader : XmiElementReader<Auriga.Capellacore.IStringPropertyValue>, IXmiElementReader<Auriga.Capellacore.IStringPropertyValue>
    {
        ...
        public StringPropertyValueReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacore.IStringPropertyValue Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of StringPropertyValue at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
