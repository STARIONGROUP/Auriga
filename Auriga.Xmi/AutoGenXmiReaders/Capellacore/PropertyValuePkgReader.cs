...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacore
{
    ...
    public class PropertyValuePkgReader : XmiElementReader<Auriga.Capellacore.IPropertyValuePkg>, IXmiElementReader<Auriga.Capellacore.IPropertyValuePkg>
    {
        ...
        public Auriga.Capellacore.IPropertyValuePkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        break;
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PropertyValuePkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
