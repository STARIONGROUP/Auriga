...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacore
{
    ...
    public class PropertyValueGroupReader : XmiElementReader<Auriga.Capellacore.IPropertyValueGroup>, IXmiElementReader<Auriga.Capellacore.IPropertyValueGroup>
    {
        ...
        public Auriga.Capellacore.IPropertyValueGroup Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PropertyValueGroup at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
