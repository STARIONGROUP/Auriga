...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacore
{
    ...
    public class FloatPropertyValueReader : XmiElementReader<Auriga.Capellacore.IFloatPropertyValue>, IXmiElementReader<Auriga.Capellacore.IFloatPropertyValue>
    {
        ...
        public FloatPropertyValueReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacore.IFloatPropertyValue Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { var raw = xmlReader.GetAttribute("value"); if (!string.IsNullOrEmpty(raw) && float.TryParse(raw, System.Globalization.NumberStyles.Float, System.Globalization.CultureInfo.InvariantCulture, out var parsed)) { poco.Value = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of FloatPropertyValue at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
