...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Re
{
    ...
    public class CompliancyDefinitionPkgReader : XmiElementReader<Auriga.Model.Re.ICompliancyDefinitionPkg>, IXmiElementReader<Auriga.Model.Re.ICompliancyDefinitionPkg>
    {
        ...
        public CompliancyDefinitionPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Re.ICompliancyDefinitionPkg"/></returns>
        public Auriga.Model.Re.ICompliancyDefinitionPkg Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Re.CompliancyDefinitionPkg();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading CompliancyDefinitionPkg at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            case "ownedDefinitions":
                            {
                                ...
                                {
                                    CollectMultiValueReferences(poco, "OwnedDefinitions", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedDefinitions.Add((Auriga.Model.Re.ICompliancyDefinition)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CompliancyDefinitionPkgReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by CompliancyDefinitionPkgReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read CompliancyDefinitionPkg but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
