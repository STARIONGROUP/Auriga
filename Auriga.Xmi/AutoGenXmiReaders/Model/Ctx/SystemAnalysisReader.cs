...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Ctx
{
    ...
    /// </summary>
    public class SystemAnalysisReader : XmiElementReader<Auriga.Model.Ctx.ISystemAnalysis>, IXmiElementReader<Auriga.Model.Ctx.ISystemAnalysis>
    {
        /// <summary>
        ...
        public SystemAnalysisReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Ctx.ISystemAnalysis"/></returns>
        public Auriga.Model.Ctx.ISystemAnalysis Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Ctx.SystemAnalysis();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading SystemAnalysis at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedFunctionPkg":
                            {
                                ...
                                {
                                    poco.OwnedFunctionPkg = (Auriga.Model.Fa.IFunctionPkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "ownedFunctionalAllocations":
                            {
                                ...
                                {
                                    poco.OwnedFunctionalAllocations.Add((Auriga.Model.Fa.IComponentFunctionalAllocation)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedMissionPkg":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "OwnedMissionPkg", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedMissionPkg = (Auriga.Model.Ctx.IMissionPkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "ownedOperationalAnalysisRealizations":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedOperationalAnalysisRealizations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedOperationalAnalysisRealizations.Add((Auriga.Model.Ctx.IOperationalAnalysisRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedSystemComponentPkg":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "OwnedSystemComponentPkg", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedSystemComponentPkg = (Auriga.Model.Ctx.ISystemComponentPkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"SystemAnalysisReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by SystemAnalysisReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
