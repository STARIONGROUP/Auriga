// ------------------------------------------------------------------------------------------------
// <copyright file="LogicalFunctionPkgReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.La
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>LogicalFunctionPkg</c> from its
    ...
    public class LogicalFunctionPkgReader : XmiElementReader<Auriga.Model.La.ILogicalFunctionPkg>, IXmiElementReader<Auriga.Model.La.ILogicalFunctionPkg>
    {
        ...
        public LogicalFunctionPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>LogicalFunctionPkg</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.La.ILogicalFunctionPkg"/></returns>
        public Auriga.Model.La.ILogicalFunctionPkg Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.La.LogicalFunctionPkg();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading LogicalFunctionPkg at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedLogicalFunctionPkgs":
                            {
                                ...
                                {
                                    poco.OwnedLogicalFunctionPkgs.Add((Auriga.Model.La.ILogicalFunctionPkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }

                                break;
                            }
                            case "ownedLogicalFunctions":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedLogicalFunctions", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedLogicalFunctions.Add((Auriga.Model.La.ILogicalFunction)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"LogicalFunctionPkgReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by LogicalFunctionPkgReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read LogicalFunctionPkg but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
