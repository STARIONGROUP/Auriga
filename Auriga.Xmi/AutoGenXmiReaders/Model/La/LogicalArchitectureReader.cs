...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.La
{
    ...
    /// </summary>
    public class LogicalArchitectureReader : XmiElementReader<Auriga.Model.La.ILogicalArchitecture>, IXmiElementReader<Auriga.Model.La.ILogicalArchitecture>
    {
        /// <summary>
        ...
        public LogicalArchitectureReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.La.ILogicalArchitecture"/></returns>
        public Auriga.Model.La.ILogicalArchitecture Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.La.LogicalArchitecture();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading LogicalArchitecture at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedLogicalComponentPkg":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "OwnedLogicalComponentPkg", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedLogicalComponentPkg = (Auriga.Model.La.ILogicalComponentPkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "ownedSystemAnalysisRealizations":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedSystemAnalysisRealizations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedSystemAnalysisRealizations.Add((Auriga.Model.La.ISystemAnalysisRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"LogicalArchitectureReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by LogicalArchitectureReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
