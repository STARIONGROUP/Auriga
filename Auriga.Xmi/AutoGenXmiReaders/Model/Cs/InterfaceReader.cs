...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Cs
{
    ...
    /// </summary>
    public class InterfaceReader : XmiElementReader<Auriga.Model.Cs.IInterface>, IXmiElementReader<Auriga.Model.Cs.IInterface>
    {
        /// <summary>
        ...
        public InterfaceReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Cs.IInterface"/></returns>
        public Auriga.Model.Cs.IInterface Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Cs.Interface();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                {
                    var raw = xmlReader.GetAttribute("structural");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.Structural = parsed;
                    }
                }
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedExchangeItemAllocations":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedExchangeItemAllocations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedExchangeItemAllocations.Add((Auriga.Model.Cs.IExchangeItemAllocation)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                this.Logger.LogWarning("Not supported by InterfaceReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read Interface but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
