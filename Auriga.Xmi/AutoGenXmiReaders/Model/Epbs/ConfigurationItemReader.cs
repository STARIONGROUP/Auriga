...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Epbs
{
    ...
    /// </summary>
    public class ConfigurationItemReader : XmiElementReader<Auriga.Model.Epbs.IConfigurationItem>, IXmiElementReader<Auriga.Model.Epbs.IConfigurationItem>
    {
        /// <summary>
        ...
        public ConfigurationItemReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Epbs.IConfigurationItem"/></returns>
        public Auriga.Model.Epbs.IConfigurationItem Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Epbs.ConfigurationItem();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading ConfigurationItem at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedPhysicalArtifactRealizations":
                            {
                                ...
                                {
                                    poco.OwnedPhysicalArtifactRealizations.Add((Auriga.Model.Epbs.IPhysicalArtifactRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ConfigurationItemReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ConfigurationItemReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
