...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Epbs
{
    ...
    /// </summary>
    public class EPBSArchitectureReader : XmiElementReader<Auriga.Model.Epbs.IEPBSArchitecture>, IXmiElementReader<Auriga.Model.Epbs.IEPBSArchitecture>
    {
        /// <summary>
        ...
        public EPBSArchitectureReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Epbs.IEPBSArchitecture"/></returns>
        public Auriga.Model.Epbs.IEPBSArchitecture Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Epbs.EPBSArchitecture();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading EPBSArchitecture at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedConfigurationItemPkg":
                            {
                                ...
                                {
                                    poco.OwnedConfigurationItemPkg = (Auriga.Model.Epbs.IConfigurationItemPkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "ownedPhysicalArchitectureRealizations":
                            {
                                ...
                                {
                                    poco.OwnedPhysicalArchitectureRealizations.Add((Auriga.Model.Epbs.IPhysicalArchitectureRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"EPBSArchitectureReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by EPBSArchitectureReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
