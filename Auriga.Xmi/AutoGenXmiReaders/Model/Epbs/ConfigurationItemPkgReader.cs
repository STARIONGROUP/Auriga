...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Epbs
{
    using System;
    using System.Xml;
    ...
    public class ConfigurationItemPkgReader : XmiElementReader<Auriga.Model.Epbs.IConfigurationItemPkg>, IXmiElementReader<Auriga.Model.Epbs.IConfigurationItemPkg>
    {
        ...
        public ConfigurationItemPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Epbs.IConfigurationItemPkg"/></returns>
        public Auriga.Model.Epbs.IConfigurationItemPkg Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Epbs.ConfigurationItemPkg();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading ConfigurationItemPkg at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedConfigurationItemPkgs":
                            {
                                ...
                                {
                                    poco.OwnedConfigurationItemPkgs.Add((Auriga.Model.Epbs.IConfigurationItemPkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedConfigurationItems":
                            {
                                ...
                                {
                                    poco.OwnedConfigurationItems.Add((Auriga.Model.Epbs.IConfigurationItem)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ConfigurationItemPkgReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ConfigurationItemPkgReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
