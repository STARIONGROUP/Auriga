// ------------------------------------------------------------------------------------------------
// <copyright file="ComplexValueReferenceReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Information.Datavalue
{
    using System;
    using System.Xml;
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>ComplexValueReference</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class ComplexValueReferenceReader : XmiElementReader<Auriga.Model.Information.Datavalue.IComplexValueReference>, IXmiElementReader<Auriga.Model.Information.Datavalue.IComplexValueReference>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ComplexValueReferenceReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public ComplexValueReferenceReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>ComplexValueReference</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Information.Datavalue.IComplexValueReference"/></returns>
        public Auriga.Model.Information.Datavalue.IComplexValueReference Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Information.Datavalue.ComplexValueReference();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading ComplexValueReference at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectSingleValueReference(poco, "ReferencedProperty", xmlReader.GetAttribute("referencedProperty"));
                CollectSingleValueReference(poco, "ReferencedValue", xmlReader.GetAttribute("referencedValue"));
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "referencedProperty":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "ReferencedProperty", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "referencedValue":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "ReferencedValue", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ComplexValueReferenceReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ComplexValueReferenceReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read ComplexValueReference but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
