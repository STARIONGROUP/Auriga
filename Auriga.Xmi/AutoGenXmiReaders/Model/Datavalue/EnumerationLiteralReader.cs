...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Information.Datavalue
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>EnumerationLiteral</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class EnumerationLiteralReader : XmiElementReader<Auriga.Model.Information.Datavalue.IEnumerationLiteral>, IXmiElementReader<Auriga.Model.Information.Datavalue.IEnumerationLiteral>
    {
        /// <summary>
        ...
        public EnumerationLiteralReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>EnumerationLiteral</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Information.Datavalue.IEnumerationLiteral"/></returns>
        public Auriga.Model.Information.Datavalue.IEnumerationLiteral Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Information.Datavalue.EnumerationLiteral();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading EnumerationLiteral at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "domainValue":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "DomainValue", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.DomainValue = (Auriga.Model.Information.Datavalue.IDataValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"EnumerationLiteralReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by EnumerationLiteralReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read EnumerationLiteral but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
