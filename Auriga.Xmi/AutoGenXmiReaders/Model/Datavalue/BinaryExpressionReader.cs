...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Information.Datavalue
{
    ...
    /// </summary>
    public class BinaryExpressionReader : XmiElementReader<Auriga.Model.Information.Datavalue.IBinaryExpression>, IXmiElementReader<Auriga.Model.Information.Datavalue.IBinaryExpression>
    {
        /// <summary>
        ...
        public BinaryExpressionReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Information.Datavalue.IBinaryExpression"/></returns>
        public Auriga.Model.Information.Datavalue.IBinaryExpression Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Information.Datavalue.BinaryExpression();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading BinaryExpression at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                poco.Name = xmlReader.GetAttribute("name");
                {
                    ...
                    {
                        if (Auriga.Extensions.BinaryOperatorProvider.TryParse(raw.AsSpan(), out var parsed))
                        {
                            poco.Operator = parsed;
                        }
                        else
                        {
                            this.HandleUnknownEnumLiteral("BinaryOperator", "operator", raw, xmlLineInfo);
                        }
                    }
                }
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedLeftOperand":
                            {
                                ...
                                {
                                    poco.OwnedLeftOperand = (Auriga.Model.Information.Datavalue.IDataValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "ownedRightOperand":
                            {
                                ...
                                {
                                    poco.OwnedRightOperand = (Auriga.Model.Information.Datavalue.IDataValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"BinaryExpressionReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by BinaryExpressionReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
