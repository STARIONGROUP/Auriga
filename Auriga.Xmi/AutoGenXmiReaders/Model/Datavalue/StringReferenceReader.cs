// ------------------------------------------------------------------------------------------------
// <copyright file="StringReferenceReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Information.Datavalue
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>StringReference</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class StringReferenceReader : XmiElementReader<Auriga.Model.Information.Datavalue.IStringReference>, IXmiElementReader<Auriga.Model.Information.Datavalue.IStringReference>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="StringReferenceReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public StringReferenceReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Information.Datavalue.IStringReference"/></returns>
        public Auriga.Model.Information.Datavalue.IStringReference Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Information.Datavalue.StringReference();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading StringReference at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
            ...
            {
                this.Logger.LogWarning("Expected an element to read StringReference but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
