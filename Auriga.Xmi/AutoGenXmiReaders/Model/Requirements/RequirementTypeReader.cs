...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Requirements
{
    ...
    /// </summary>
    public class RequirementTypeReader : XmiElementReader<Auriga.Model.Requirements.IRequirementType>, IXmiElementReader<Auriga.Model.Requirements.IRequirementType>
    {
        /// <summary>
        ...
        public RequirementTypeReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Requirements.IRequirementType"/></returns>
        public Auriga.Model.Requirements.IRequirementType Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Requirements.RequirementType();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading RequirementType at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"RequirementTypeReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by RequirementTypeReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read RequirementType but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
