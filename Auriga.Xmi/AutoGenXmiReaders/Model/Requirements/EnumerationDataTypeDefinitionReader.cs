...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Requirements
{
    ...
    /// </summary>
    public class EnumerationDataTypeDefinitionReader : XmiElementReader<Auriga.Model.Requirements.IEnumerationDataTypeDefinition>, IXmiElementReader<Auriga.Model.Requirements.IEnumerationDataTypeDefinition>
    {
        /// <summary>
        ...
        public EnumerationDataTypeDefinitionReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Requirements.IEnumerationDataTypeDefinition"/></returns>
        public Auriga.Model.Requirements.IEnumerationDataTypeDefinition Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Requirements.EnumerationDataTypeDefinition();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading EnumerationDataTypeDefinition at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "specifiedValues":
                            {
                                ...
                                {
                                    poco.SpecifiedValues.Add((Auriga.Model.Requirements.IEnumValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"EnumerationDataTypeDefinitionReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by EnumerationDataTypeDefinitionReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
