...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Requirements
{
    ...
    /// </summary>
    public class AttributeDefinitionEnumerationReader : XmiElementReader<Auriga.Model.Requirements.IAttributeDefinitionEnumeration>, IXmiElementReader<Auriga.Model.Requirements.IAttributeDefinitionEnumeration>
    {
        /// <summary>
        ...
        public AttributeDefinitionEnumerationReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Requirements.IAttributeDefinitionEnumeration"/></returns>
        public Auriga.Model.Requirements.IAttributeDefinitionEnumeration Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Requirements.AttributeDefinitionEnumeration();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading AttributeDefinitionEnumeration at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    var raw = xmlReader.GetAttribute("multiValued");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.MultiValued = parsed;
                    }
                }
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            case "defaultValue":
                            {
                                ...
                                {
                                    CollectSingleValueReference(poco, "DefaultValue", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.DefaultValue = (Auriga.Model.Requirements.IAttribute)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "definitionType":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "DefinitionType", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"AttributeDefinitionEnumerationReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by AttributeDefinitionEnumerationReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read AttributeDefinitionEnumeration but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
