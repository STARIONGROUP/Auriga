...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Oa
{
    ...
    /// </summary>
    public class OperationalActivityPkgReader : XmiElementReader<Auriga.Model.Oa.IOperationalActivityPkg>, IXmiElementReader<Auriga.Model.Oa.IOperationalActivityPkg>
    {
        /// <summary>
        ...
        public OperationalActivityPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Oa.IOperationalActivityPkg"/></returns>
        public Auriga.Model.Oa.IOperationalActivityPkg Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Oa.OperationalActivityPkg();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading OperationalActivityPkg at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedOperationalActivities":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedOperationalActivities", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedOperationalActivities.Add((Auriga.Model.Oa.IOperationalActivity)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"OperationalActivityPkgReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by OperationalActivityPkgReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
