...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Oa
{
    ...
    public class SwimlaneReader : XmiElementReader<Auriga.Model.Oa.ISwimlane>, IXmiElementReader<Auriga.Model.Oa.ISwimlane>
    {
        ...
        public SwimlaneReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Oa.ISwimlane"/></returns>
        public Auriga.Model.Oa.ISwimlane Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Oa.Swimlane();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading Swimlane at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    var raw = xmlReader.GetAttribute("isDimension");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.IsDimension = parsed;
                    }
                }
                {
                    var raw = xmlReader.GetAttribute("isExternal");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.IsExternal = parsed;
                    }
                }
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedEdges":
                            {
                                ...
                                {
                                    poco.OwnedEdges.Add((Auriga.Model.Activity.IActivityEdge)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedNodes":
                            {
                                ...
                                {
                                    poco.OwnedNodes.Add((Auriga.Model.Activity.IActivityNode)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "representedElement":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "RepresentedElement", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "subGroups":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "SubGroups", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.SubGroups.Add((Auriga.Model.Activity.IActivityGroup)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "superGroup":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "SuperGroup", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"SwimlaneReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by SwimlaneReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
