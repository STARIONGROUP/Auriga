...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Capellamodeller
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>Project</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class ProjectReader : XmiElementReader<Auriga.Model.Capellamodeller.IProject>, IXmiElementReader<Auriga.Model.Capellamodeller.IProject>
    {
        /// <summary>
        ...
        public ProjectReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>Project</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Capellamodeller.IProject"/></returns>
        public Auriga.Model.Capellamodeller.IProject Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Capellamodeller.Project();
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "keyValuePairs":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "KeyValuePairs", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.KeyValuePairs.Add((Auriga.Model.Capellacore.IKeyValue)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedFolders":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedFolders", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedFolders.Add((Auriga.Model.Capellamodeller.IFolder)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedModelRoots":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedModelRoots", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedModelRoots.Add((Auriga.Model.Capellamodeller.IModelRoot)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                this.Logger.LogWarning("Not supported by ProjectReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read Project but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
