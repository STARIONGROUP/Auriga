...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Capellamodeller
{
    ...
    /// </summary>
    public class FolderReader : XmiElementReader<Auriga.Model.Capellamodeller.IFolder>, IXmiElementReader<Auriga.Model.Capellamodeller.IFolder>
    {
        /// <summary>
        ...
        public FolderReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Capellamodeller.IFolder"/></returns>
        public Auriga.Model.Capellamodeller.IFolder Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Capellamodeller.Folder();
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"FolderReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by FolderReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
