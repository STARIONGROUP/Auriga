...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Pa
{
    ...
    /// </summary>
    public class PhysicalComponentPkgReader : XmiElementReader<Auriga.Model.Pa.IPhysicalComponentPkg>, IXmiElementReader<Auriga.Model.Pa.IPhysicalComponentPkg>
    {
        /// <summary>
        ...
        public PhysicalComponentPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Pa.IPhysicalComponentPkg"/></returns>
        public Auriga.Model.Pa.IPhysicalComponentPkg Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Pa.PhysicalComponentPkg();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading PhysicalComponentPkg at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedDeployments":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedDeployments", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedDeployments.Add((Auriga.Model.Cs.IAbstractDeploymentLink)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedPhysicalComponentPkgs":
                            {
                                ...
                                {
                                    poco.OwnedPhysicalComponentPkgs.Add((Auriga.Model.Pa.IPhysicalComponentPkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedPhysicalComponents":
                            {
                                ...
                                {
                                    poco.OwnedPhysicalComponents.Add((Auriga.Model.Pa.IPhysicalComponent)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"PhysicalComponentPkgReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by PhysicalComponentPkgReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
