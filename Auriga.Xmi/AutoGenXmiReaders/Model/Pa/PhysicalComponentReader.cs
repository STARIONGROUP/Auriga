...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Pa
{
    ...
    /// </summary>
    public class PhysicalComponentReader : XmiElementReader<Auriga.Model.Pa.IPhysicalComponent>, IXmiElementReader<Auriga.Model.Pa.IPhysicalComponent>
    {
        /// <summary>
        ...
        public PhysicalComponentReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Pa.IPhysicalComponent"/></returns>
        public Auriga.Model.Pa.IPhysicalComponent Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Pa.PhysicalComponent();
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"PhysicalComponentReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by PhysicalComponentReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
