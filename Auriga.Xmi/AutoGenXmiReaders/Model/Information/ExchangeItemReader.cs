...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Information
{
    ...
    /// </summary>
    public class ExchangeItemReader : XmiElementReader<Auriga.Model.Information.IExchangeItem>, IXmiElementReader<Auriga.Model.Information.IExchangeItem>
    {
        /// <summary>
        ...
        public ExchangeItemReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Information.IExchangeItem"/></returns>
        public Auriga.Model.Information.IExchangeItem Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Information.ExchangeItem();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading ExchangeItem at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedElements":
                            {
                                ...
                                {
                                    poco.OwnedElements.Add((Auriga.Model.Information.IExchangeItemElement)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedExchangeItemInstances":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedExchangeItemInstances", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedExchangeItemInstances.Add((Auriga.Model.Information.IExchangeItemInstance)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ExchangeItemReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ExchangeItemReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
