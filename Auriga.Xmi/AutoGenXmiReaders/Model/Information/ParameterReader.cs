...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Information
{
    ...
    /// </summary>
    public class ParameterReader : XmiElementReader<Auriga.Model.Information.IParameter>, IXmiElementReader<Auriga.Model.Information.IParameter>
    {
        /// <summary>
        ...
        public ParameterReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Information.IParameter"/></returns>
        public Auriga.Model.Information.IParameter Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Information.Parameter();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading Parameter at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    var raw = xmlReader.GetAttribute("isException");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.IsException = parsed;
                    }
                }
                {
                    var raw = xmlReader.GetAttribute("isOptional");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.IsOptional = parsed;
                    }
                }
                {
                    var raw = xmlReader.GetAttribute("isStream");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.IsStream = parsed;
                    }
                }
                {
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "parameterSet":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "ParameterSet", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "probability":
                            {
                                ...
                                {
                                    poco.Probability = (Auriga.Model.Modellingcore.IValueSpecification)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            case "rate":
                            {
                                ...
                                {
                                    poco.Rate = (Auriga.Model.Modellingcore.IValueSpecification)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ParameterReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ParameterReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
