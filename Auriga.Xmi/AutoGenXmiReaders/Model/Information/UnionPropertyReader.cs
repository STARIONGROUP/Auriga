// ------------------------------------------------------------------------------------------------
// <copyright file="UnionPropertyReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Information
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>UnionProperty</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class UnionPropertyReader : XmiElementReader<Auriga.Model.Information.IUnionProperty>, IXmiElementReader<Auriga.Model.Information.IUnionProperty>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="UnionPropertyReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public UnionPropertyReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Information.IUnionProperty"/></returns>
        public Auriga.Model.Information.IUnionProperty Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Information.UnionProperty();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading UnionProperty at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectMultiValueReferences(poco, "Qualifier", xmlReader.GetAttribute("qualifier"));
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "qualifier":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "Qualifier", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
            ...
            {
                this.Logger.LogWarning("Expected an element to read UnionProperty but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
