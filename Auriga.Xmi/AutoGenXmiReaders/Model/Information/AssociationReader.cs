...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Information
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>Association</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class AssociationReader : XmiElementReader<Auriga.Model.Information.IAssociation>, IXmiElementReader<Auriga.Model.Information.IAssociation>
    {
        /// <summary>
        ...
        public AssociationReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>Association</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Information.IAssociation"/></returns>
        public Auriga.Model.Information.IAssociation Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Information.Association();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                CollectMultiValueReferences(poco, "NavigableMembers", xmlReader.GetAttribute("navigableMembers"));
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "navigableMembers":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "NavigableMembers", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "ownedMembers":
                            {
                                ...
                                {
                                    poco.OwnedMembers.Add((Auriga.Model.Information.IProperty)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"AssociationReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by AssociationReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read Association but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
