...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Information
{
    ...
    /// </summary>
    public class ServiceReader : XmiElementReader<Auriga.Model.Information.IService>, IXmiElementReader<Auriga.Model.Information.IService>
    {
        /// <summary>
        ...
        public ServiceReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Information.IService"/></returns>
        public Auriga.Model.Information.IService Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Information.Service();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading Service at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "messageReferences":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "MessageReferences", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "ownedExchangeItemRealizations":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedExchangeItemRealizations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedExchangeItemRealizations.Add((Auriga.Model.Information.IExchangeItemRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedOperationAllocation":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedOperationAllocation", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedOperationAllocation.Add((Auriga.Model.Information.IOperationAllocation)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedParameters":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedParameters", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedParameters.Add((Auriga.Model.Information.IParameter)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "thrownExceptions":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "ThrownExceptions", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ServiceReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ServiceReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
