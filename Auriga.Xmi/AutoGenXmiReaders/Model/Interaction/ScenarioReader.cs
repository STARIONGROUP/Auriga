...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Interaction
{
    using System;
    using System.Xml;
    ...
    public class ScenarioReader : XmiElementReader<Auriga.Model.Interaction.IScenario>, IXmiElementReader<Auriga.Model.Interaction.IScenario>
    {
        ...
        public ScenarioReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Interaction.IScenario"/></returns>
        public Auriga.Model.Interaction.IScenario Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Interaction.Scenario();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading Scenario at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    var raw = xmlReader.GetAttribute("merged");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.Merged = parsed;
                    }
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedConstraintDurations":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedConstraintDurations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedConstraintDurations.Add((Auriga.Model.Interaction.IConstraintDuration)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedEvents":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedEvents", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedEvents.Add((Auriga.Model.Interaction.IEvent)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedFormalGates":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedFormalGates", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedFormalGates.Add((Auriga.Model.Interaction.IGate)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedInstanceRoles":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedInstanceRoles", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedInstanceRoles.Add((Auriga.Model.Interaction.IInstanceRole)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedInteractionFragments":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedInteractionFragments", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedInteractionFragments.Add((Auriga.Model.Interaction.IInteractionFragment)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedMessages":
                            {
                                ...
                                {
                                    poco.OwnedMessages.Add((Auriga.Model.Interaction.ISequenceMessage)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedScenarioRealization":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedScenarioRealization", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedScenarioRealization.Add((Auriga.Model.Interaction.IScenarioRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedTimeLapses":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedTimeLapses", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedTimeLapses.Add((Auriga.Model.Interaction.ITimeLapse)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ScenarioReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ScenarioReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
