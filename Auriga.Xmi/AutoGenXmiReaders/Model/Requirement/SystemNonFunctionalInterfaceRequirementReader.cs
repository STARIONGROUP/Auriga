// ------------------------------------------------------------------------------------------------
// <copyright file="SystemNonFunctionalInterfaceRequirementReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Requirement
{
    using System;
    using System.Xml;
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>SystemNonFunctionalInterfaceRequirement</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class SystemNonFunctionalInterfaceRequirementReader : XmiElementReader<Auriga.Model.Requirement.ISystemNonFunctionalInterfaceRequirement>, IXmiElementReader<Auriga.Model.Requirement.ISystemNonFunctionalInterfaceRequirement>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="SystemNonFunctionalInterfaceRequirementReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public SystemNonFunctionalInterfaceRequirementReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>SystemNonFunctionalInterfaceRequirement</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Requirement.ISystemNonFunctionalInterfaceRequirement"/></returns>
        public Auriga.Model.Requirement.ISystemNonFunctionalInterfaceRequirement Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Requirement.SystemNonFunctionalInterfaceRequirement();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading SystemNonFunctionalInterfaceRequirement at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                poco.AdditionalInformation = xmlReader.GetAttribute("additionalInformation");
                ...
                poco.Feature = xmlReader.GetAttribute("feature");
                CollectMultiValueReferences(poco, "Features", xmlReader.GetAttribute("features"));
                poco.ImplementationVersion = xmlReader.GetAttribute("implementationVersion");
                {
                    var raw = xmlReader.GetAttribute("isObsolete");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.IsObsolete = parsed;
                    }
                }
                poco.Name = xmlReader.GetAttribute("name");
                poco.RequirementId = xmlReader.GetAttribute("requirementId");
                ...
                poco.VerificationMethod = xmlReader.GetAttribute("verificationMethod");
                poco.VerificationPhase = xmlReader.GetAttribute("verificationPhase");
                {
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"SystemNonFunctionalInterfaceRequirementReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by SystemNonFunctionalInterfaceRequirementReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read SystemNonFunctionalInterfaceRequirement but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
