...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Capellacommon
{
    using System;
    using System.Xml;
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>FinalState</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class FinalStateReader : XmiElementReader<Auriga.Model.Capellacommon.IFinalState>, IXmiElementReader<Auriga.Model.Capellacommon.IFinalState>
    {
        /// <summary>
        ...
        public FinalStateReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>FinalState</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Capellacommon.IFinalState"/></returns>
        public Auriga.Model.Capellacommon.IFinalState Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Capellacommon.FinalState();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading FinalState at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectMultiValueReferences(poco, "DoActivity", xmlReader.GetAttribute("doActivity"));
                CollectMultiValueReferences(poco, "Entry", xmlReader.GetAttribute("entry"));
                CollectMultiValueReferences(poco, "Exit", xmlReader.GetAttribute("exit"));
                CollectMultiValueReferences(poco, "ExploitedStates", xmlReader.GetAttribute("exploitedStates"));
                CollectMultiValueReferences(poco, "Features", xmlReader.GetAttribute("features"));
                poco.Name = xmlReader.GetAttribute("name");
                CollectMultiValueReferences(poco, "ReferencedStates", xmlReader.GetAttribute("referencedStates"));
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "doActivity":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "DoActivity", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "entry":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "Entry", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "exit":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "Exit", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "ownedAbstractStateRealizations":
                            {
                                ...
                                {
                                    poco.OwnedAbstractStateRealizations.Add((Auriga.Model.Capellacommon.IAbstractStateRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }

                                break;
                            }
                            case "ownedConnectionPoints":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedConnectionPoints", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedConnectionPoints.Add((Auriga.Model.Capellacommon.IPseudostate)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedRegions":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedRegions", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedRegions.Add((Auriga.Model.Capellacommon.IRegion)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "stateInvariant":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "StateInvariant", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.StateInvariant = (Auriga.Model.Modellingcore.IAbstractConstraint)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"FinalStateReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by FinalStateReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read FinalState but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
