...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Sharedmodel
{
    using System;
    using System.Xml;
    ...
    /// </summary>
    public class GenericPkgReader : XmiElementReader<Auriga.Model.Sharedmodel.IGenericPkg>, IXmiElementReader<Auriga.Model.Sharedmodel.IGenericPkg>
    {
        /// <summary>
        ...
        public GenericPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Sharedmodel.IGenericPkg"/></returns>
        public Auriga.Model.Sharedmodel.IGenericPkg Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Sharedmodel.GenericPkg();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading GenericPkg at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "capellaElements":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "CapellaElements", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.CapellaElements.Add((Auriga.Model.Capellacore.ICapellaElement)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "subGenericPkgs":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "SubGenericPkgs", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.SubGenericPkgs.Add((Auriga.Model.Sharedmodel.IGenericPkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"GenericPkgReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by GenericPkgReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read GenericPkg but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
