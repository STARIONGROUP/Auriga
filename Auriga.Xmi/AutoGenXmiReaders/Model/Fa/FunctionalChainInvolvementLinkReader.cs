// ------------------------------------------------------------------------------------------------
// <copyright file="FunctionalChainInvolvementLinkReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Fa
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>FunctionalChainInvolvementLink</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class FunctionalChainInvolvementLinkReader : XmiElementReader<Auriga.Model.Fa.IFunctionalChainInvolvementLink>, IXmiElementReader<Auriga.Model.Fa.IFunctionalChainInvolvementLink>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="FunctionalChainInvolvementLinkReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public FunctionalChainInvolvementLinkReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>FunctionalChainInvolvementLink</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Fa.IFunctionalChainInvolvementLink"/></returns>
        public Auriga.Model.Fa.IFunctionalChainInvolvementLink Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Fa.FunctionalChainInvolvementLink();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading FunctionalChainInvolvementLink at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectSingleValueReference(poco, "ExchangeContext", xmlReader.GetAttribute("exchangeContext"));
                ...
                CollectSingleValueReference(poco, "Involved", xmlReader.GetAttribute("involved"));
                ...
                CollectMultiValueReferences(poco, "SourceReferenceHierarchy", xmlReader.GetAttribute("sourceReferenceHierarchy"));
                ...
                CollectMultiValueReferences(poco, "TargetReferenceHierarchy", xmlReader.GetAttribute("targetReferenceHierarchy"));
                {
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "exchangeContext":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "ExchangeContext", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "involved":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "Involved", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "sourceReferenceHierarchy":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "SourceReferenceHierarchy", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "targetReferenceHierarchy":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "TargetReferenceHierarchy", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"FunctionalChainInvolvementLinkReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by FunctionalChainInvolvementLinkReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read FunctionalChainInvolvementLink but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
