// ------------------------------------------------------------------------------------------------
// <copyright file="ComponentExchangeReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Fa
{
    using System;
    using System.Xml;
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>ComponentExchange</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class ComponentExchangeReader : XmiElementReader<Auriga.Model.Fa.IComponentExchange>, IXmiElementReader<Auriga.Model.Fa.IComponentExchange>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ComponentExchangeReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public ComponentExchangeReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>ComponentExchange</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Fa.IComponentExchange"/></returns>
        public Auriga.Model.Fa.IComponentExchange Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Fa.ComponentExchange();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading ComponentExchange at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectMultiValueReferences(poco, "ConvoyedInformations", xmlReader.GetAttribute("convoyedInformations"));
                ...
                CollectMultiValueReferences(poco, "Features", xmlReader.GetAttribute("features"));
                {
                    ...
                    {
                        if (Auriga.Extensions.ComponentExchangeKindProvider.TryParse(raw.AsSpan(), out var parsed))
                        {
                            poco.Kind = parsed;
                        }
                        else
                        {
                            this.HandleUnknownEnumLiteral("ComponentExchangeKind", "kind", raw, xmlLineInfo);
                        }
                    }
                }
                CollectSingleValueReference(poco, "Link", xmlReader.GetAttribute("link"));
                poco.Name = xmlReader.GetAttribute("name");
                {
                    var raw = xmlReader.GetAttribute("oriented");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.Oriented = parsed;
                    }
                }
                CollectMultiValueReferences(poco, "Realizations", xmlReader.GetAttribute("realizations"));
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "convoyedInformations":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "ConvoyedInformations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "link":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "Link", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "ownedComponentExchangeEnds":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedComponentExchangeEnds", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedComponentExchangeEnds.Add((Auriga.Model.Fa.IComponentExchangeEnd)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }

                                break;
                            }
                            case "ownedComponentExchangeFunctionalExchangeAllocations":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedComponentExchangeFunctionalExchangeAllocations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedComponentExchangeFunctionalExchangeAllocations.Add((Auriga.Model.Fa.IComponentExchangeFunctionalExchangeAllocation)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedComponentExchangeRealizations":
                            {
                                ...
                                {
                                    poco.OwnedComponentExchangeRealizations.Add((Auriga.Model.Fa.IComponentExchangeRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "realizations":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "Realizations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ComponentExchangeReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ComponentExchangeReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read ComponentExchange but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
