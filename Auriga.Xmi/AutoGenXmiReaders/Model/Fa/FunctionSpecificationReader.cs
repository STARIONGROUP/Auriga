...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Fa
{
    ...
    public class FunctionSpecificationReader : XmiElementReader<Auriga.Model.Fa.IFunctionSpecification>, IXmiElementReader<Auriga.Model.Fa.IFunctionSpecification>
    {
        ...
        public FunctionSpecificationReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.Fa.IFunctionSpecification"/></returns>
        public Auriga.Model.Fa.IFunctionSpecification Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Fa.FunctionSpecification();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading FunctionSpecification at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedFunctionPorts":
                            {
                                ...
                                {
                                    poco.OwnedFunctionPorts.Add((Auriga.Model.Fa.IFunctionPort)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedGroups":
                            {
                                ...
                                {
                                    poco.OwnedGroups.Add((Auriga.Model.Activity.IActivityGroup)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"FunctionSpecificationReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by FunctionSpecificationReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
