...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.CapellaRequirements
{
    ...
    /// </summary>
    public class CapellaModuleReader : XmiElementReader<Auriga.Model.CapellaRequirements.ICapellaModule>, IXmiElementReader<Auriga.Model.CapellaRequirements.ICapellaModule>
    {
        /// <summary>
        ...
        public CapellaModuleReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.CapellaRequirements.ICapellaModule"/></returns>
        public Auriga.Model.CapellaRequirements.ICapellaModule Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.CapellaRequirements.CapellaModule();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading CapellaModule at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "ownedAttributes":
                            {
                                ...
                                {
                                    poco.OwnedAttributes.Add((Auriga.Model.Requirements.IAttribute)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedRequirements":
                            {
                                ...
                                {
                                    poco.OwnedRequirements.Add((Auriga.Model.Requirements.IRequirement)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CapellaModuleReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by CapellaModuleReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read CapellaModule but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
