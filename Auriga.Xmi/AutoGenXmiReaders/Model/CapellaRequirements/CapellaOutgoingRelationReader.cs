...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.CapellaRequirements
{
    ...
    /// </summary>
    public class CapellaOutgoingRelationReader : XmiElementReader<Auriga.Model.CapellaRequirements.ICapellaOutgoingRelation>, IXmiElementReader<Auriga.Model.CapellaRequirements.ICapellaOutgoingRelation>
    {
        /// <summary>
        ...
        public CapellaOutgoingRelationReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.CapellaRequirements.ICapellaOutgoingRelation"/></returns>
        public Auriga.Model.CapellaRequirements.ICapellaOutgoingRelation Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.CapellaRequirements.CapellaOutgoingRelation();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading CapellaOutgoingRelation at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CapellaOutgoingRelationReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by CapellaOutgoingRelationReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read CapellaOutgoingRelation but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
