...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.CapellaRequirements
{
    ...
    /// </summary>
    public class CapellaTypesFolderReader : XmiElementReader<Auriga.Model.CapellaRequirements.ICapellaTypesFolder>, IXmiElementReader<Auriga.Model.CapellaRequirements.ICapellaTypesFolder>
    {
        /// <summary>
        ...
        public CapellaTypesFolderReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Model.CapellaRequirements.ICapellaTypesFolder"/></returns>
        public Auriga.Model.CapellaRequirements.ICapellaTypesFolder Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.CapellaRequirements.CapellaTypesFolder();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading CapellaTypesFolder at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            case "ownedDefinitionTypes":
                            {
                                ...
                                {
                                    poco.OwnedDefinitionTypes.Add((Auriga.Model.Requirements.IDataTypeDefinition)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedTypes":
                            {
                                ...
                                {
                                    poco.OwnedTypes.Add((Auriga.Model.Requirements.IAbstractType)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CapellaTypesFolderReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by CapellaTypesFolderReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read CapellaTypesFolder but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
