// ------------------------------------------------------------------------------------------------
// <copyright file="ThreatReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Cybersecurity
{
    using System;
    using System.Xml;
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>Threat</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class ThreatReader : XmiElementReader<Auriga.Model.Cybersecurity.IThreat>, IXmiElementReader<Auriga.Model.Cybersecurity.IThreat>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="ThreatReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public ThreatReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>Threat</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Cybersecurity.IThreat"/></returns>
        public Auriga.Model.Cybersecurity.IThreat Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Cybersecurity.Threat();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading Threat at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectMultiValueReferences(poco, "AddressedBy", xmlReader.GetAttribute("addressedBy"));
                ...
                CollectSingleValueReference(poco, "Kind", xmlReader.GetAttribute("kind"));
                {
                ...
                poco.Rationale = xmlReader.GetAttribute("rationale");
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            case "abstractCapabilityExtensionPoints":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "AbstractCapabilityExtensionPoints", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.AbstractCapabilityExtensionPoints.Add((Auriga.Model.Interaction.IAbstractCapabilityExtensionPoint)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "addressedBy":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "AddressedBy", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "extends":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "Extends", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.Extends.Add((Auriga.Model.Interaction.IAbstractCapabilityExtend)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "includes":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "Includes", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.Includes.Add((Auriga.Model.Interaction.IAbstractCapabilityInclude)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "kind":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "Kind", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "ownedAbstractCapabilityRealizations":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedAbstractCapabilityRealizations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedAbstractCapabilityRealizations.Add((Auriga.Model.Interaction.IAbstractCapabilityRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }

                                break;
                            }
                            case "ownedAbstractFunctionAbstractCapabilityInvolvements":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedAbstractFunctionAbstractCapabilityInvolvements", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedAbstractFunctionAbstractCapabilityInvolvements.Add((Auriga.Model.Interaction.IAbstractFunctionAbstractCapabilityInvolvement)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedFunctionalChainAbstractCapabilityInvolvements":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedFunctionalChainAbstractCapabilityInvolvements", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedFunctionalChainAbstractCapabilityInvolvements.Add((Auriga.Model.Interaction.IFunctionalChainAbstractCapabilityInvolvement)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedFunctionalChains":
                            {
                                ...
                                {
                                    poco.OwnedFunctionalChains.Add((Auriga.Model.Fa.IFunctionalChain)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedPropertyValuePkgs":
                            {
                                ...
                                {
                                    poco.OwnedPropertyValuePkgs.Add((Auriga.Model.Capellacore.IPropertyValuePkg)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedScenarios":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedScenarios", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedScenarios.Add((Auriga.Model.Interaction.IScenario)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }

                                break;
                            }
                            case "ownedThreatApplications":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedThreatApplications", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedThreatApplications.Add((Auriga.Model.Cybersecurity.IThreatApplication)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }

                                break;
                            }
                            case "ownedThreatInvolvements":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedThreatInvolvements", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedThreatInvolvements.Add((Auriga.Model.Cybersecurity.IThreatInvolvement)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "postCondition":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "PostCondition", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "preCondition":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "PreCondition", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "superGeneralizations":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "SuperGeneralizations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.SuperGeneralizations.Add((Auriga.Model.Interaction.IAbstractCapabilityGeneralization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"ThreatReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by ThreatReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read Threat but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
