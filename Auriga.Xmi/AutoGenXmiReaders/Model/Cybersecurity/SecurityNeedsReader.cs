// ------------------------------------------------------------------------------------------------
// <copyright file="SecurityNeedsReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Cybersecurity
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>SecurityNeeds</c> from its
    ...
    public class SecurityNeedsReader : XmiElementReader<Auriga.Model.Cybersecurity.ISecurityNeeds>, IXmiElementReader<Auriga.Model.Cybersecurity.ISecurityNeeds>
    {
        ...
        public SecurityNeedsReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>SecurityNeeds</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Cybersecurity.ISecurityNeeds"/></returns>
        public Auriga.Model.Cybersecurity.ISecurityNeeds Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Cybersecurity.SecurityNeeds();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading SecurityNeeds at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectSingleValueReference(poco, "AvailabilityValue", xmlReader.GetAttribute("availabilityValue"));
                CollectSingleValueReference(poco, "ConfidentialityValue", xmlReader.GetAttribute("confidentialityValue"));
                ...
                CollectSingleValueReference(poco, "IntegrityValue", xmlReader.GetAttribute("integrityValue"));
                ...
                CollectSingleValueReference(poco, "TraceabilityValue", xmlReader.GetAttribute("traceabilityValue"));
                {
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "availabilityValue":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "AvailabilityValue", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "confidentialityValue":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "ConfidentialityValue", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "integrityValue":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "IntegrityValue", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "traceabilityValue":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "TraceabilityValue", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"SecurityNeedsReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by SecurityNeedsReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read SecurityNeeds but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
