// ------------------------------------------------------------------------------------------------
// <copyright file="TrustBoundaryStorageReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Cybersecurity
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>TrustBoundaryStorage</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class TrustBoundaryStorageReader : XmiElementReader<Auriga.Model.Cybersecurity.ITrustBoundaryStorage>, IXmiElementReader<Auriga.Model.Cybersecurity.ITrustBoundaryStorage>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="TrustBoundaryStorageReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public TrustBoundaryStorageReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>TrustBoundaryStorage</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Cybersecurity.ITrustBoundaryStorage"/></returns>
        public Auriga.Model.Cybersecurity.ITrustBoundaryStorage Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Cybersecurity.TrustBoundaryStorage();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading TrustBoundaryStorage at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                {
                    var raw = xmlReader.GetAttribute("threatSource");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.ThreatSource = parsed;
                    }
                }
                {
                    var raw = xmlReader.GetAttribute("threatSourceProfile");
                    ...
                    {
                        poco.ThreatSourceProfile = parsed;
                    }
                }
                {
                    var raw = xmlReader.GetAttribute("trusted");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.Trusted = parsed;
                    }
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"TrustBoundaryStorageReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by TrustBoundaryStorageReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read TrustBoundaryStorage but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
