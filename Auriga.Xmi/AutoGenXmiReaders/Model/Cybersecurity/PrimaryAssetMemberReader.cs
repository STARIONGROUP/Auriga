// ------------------------------------------------------------------------------------------------
// <copyright file="PrimaryAssetMemberReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Cybersecurity
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>PrimaryAssetMember</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class PrimaryAssetMemberReader : XmiElementReader<Auriga.Model.Cybersecurity.IPrimaryAssetMember>, IXmiElementReader<Auriga.Model.Cybersecurity.IPrimaryAssetMember>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="PrimaryAssetMemberReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public PrimaryAssetMemberReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>PrimaryAssetMember</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Cybersecurity.IPrimaryAssetMember"/></returns>
        public Auriga.Model.Cybersecurity.IPrimaryAssetMember Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Cybersecurity.PrimaryAssetMember();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading PrimaryAssetMember at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectSingleValueReference(poco, "Member", xmlReader.GetAttribute("member"));
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "member":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "Member", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"PrimaryAssetMemberReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by PrimaryAssetMemberReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read PrimaryAssetMember but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
