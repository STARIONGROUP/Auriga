// ------------------------------------------------------------------------------------------------
// <copyright file="CybersecurityConfigurationReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.Model.AutoGenXmiReaders.Cybersecurity
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>CybersecurityConfiguration</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class CybersecurityConfigurationReader : XmiElementReader<Auriga.Model.Cybersecurity.ICybersecurityConfiguration>, IXmiElementReader<Auriga.Model.Cybersecurity.ICybersecurityConfiguration>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="CybersecurityConfigurationReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="settings">the reader settings (e.g. strict vs. lenient reading)</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public CybersecurityConfigurationReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>CybersecurityConfiguration</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Model.Cybersecurity.ICybersecurityConfiguration"/></returns>
        public Auriga.Model.Cybersecurity.ICybersecurityConfiguration Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Model.Cybersecurity.CybersecurityConfiguration();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading CybersecurityConfiguration at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectSingleValueReference(poco, "Availability", xmlReader.GetAttribute("availability"));
                CollectSingleValueReference(poco, "Confidentiality", xmlReader.GetAttribute("confidentiality"));
                ...
                CollectSingleValueReference(poco, "Integrity", xmlReader.GetAttribute("integrity"));
                ...
                CollectSingleValueReference(poco, "ThreatKind", xmlReader.GetAttribute("threatKind"));
                CollectSingleValueReference(poco, "Traceability", xmlReader.GetAttribute("traceability"));
                {
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "availability":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "Availability", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "confidentiality":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "Confidentiality", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "integrity":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "Integrity", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "threatKind":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "ThreatKind", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "traceability":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "Traceability", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"CybersecurityConfigurationReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by CybersecurityConfigurationReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read CybersecurityConfiguration but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
