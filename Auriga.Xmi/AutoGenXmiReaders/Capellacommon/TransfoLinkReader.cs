...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class TransfoLinkReader : XmiElementReader<Auriga.Capellacommon.ITransfoLink>, IXmiElementReader<Auriga.Capellacommon.ITransfoLink>
    {
        ...
        public TransfoLinkReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacommon.ITransfoLink Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of TransfoLink at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
