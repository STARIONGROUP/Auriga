...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class ChoicePseudoStateReader : XmiElementReader<Auriga.Capellacommon.IChoicePseudoState>, IXmiElementReader<Auriga.Capellacommon.IChoicePseudoState>
    {
        ...
        public Auriga.Capellacommon.IChoicePseudoState Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        break;
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ChoicePseudoState at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
