...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class ExitPointPseudoStateReader : XmiElementReader<Auriga.Capellacommon.IExitPointPseudoState>, IXmiElementReader<Auriga.Capellacommon.IExitPointPseudoState>
    {
        ...
        public Auriga.Capellacommon.IExitPointPseudoState Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        break;
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ExitPointPseudoState at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
