...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class RegionReader : XmiElementReader<Auriga.Capellacommon.IRegion>, IXmiElementReader<Auriga.Capellacommon.IRegion>
    {
        ...
        public RegionReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacommon.IRegion Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "InvolvedStates", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedStates", href); SkipElement(xmlReader); }
                            else { poco.OwnedStates.Add((Auriga.Capellacommon.IAbstractState)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedTransitions":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedTransitions", href); SkipElement(xmlReader); }
                            else { poco.OwnedTransitions.Add((Auriga.Capellacommon.IStateTransition)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Region at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
