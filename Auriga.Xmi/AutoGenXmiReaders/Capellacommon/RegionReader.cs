...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>Region</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class RegionReader : XmiElementReader<Auriga.Capellacommon.IRegion>, IXmiElementReader<Auriga.Capellacommon.IRegion>
    {
        /// <summary>
        ...
        public RegionReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>Region</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Capellacommon.IRegion"/></returns>
        public Auriga.Capellacommon.IRegion Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Capellacommon.Region();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                CollectMultiValueReferences(poco, "InvolvedStates", xmlReader.GetAttribute("involvedStates"));
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "involvedStates":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "InvolvedStates", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "ownedStates":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedStates", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedStates.Add((Auriga.Capellacommon.IAbstractState)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }

                                break;
                            }
                            case "ownedTransitions":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedTransitions", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedTransitions.Add((Auriga.Capellacommon.IStateTransition)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            default:
                                ...
                                this.Logger.LogWarning("Not supported by RegionReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read Region but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
