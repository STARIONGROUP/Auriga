...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class TimeEventReader : XmiElementReader<Auriga.Capellacommon.ITimeEvent>, IXmiElementReader<Auriga.Capellacommon.ITimeEvent>
    {
        ...
        public TimeEventReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacommon.ITimeEvent Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { if (TryParseEnum<Auriga.Capellacommon.TimeEventKind>(xmlReader.GetAttribute("kind"), out var parsed)) { poco.Kind = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of TimeEvent at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
