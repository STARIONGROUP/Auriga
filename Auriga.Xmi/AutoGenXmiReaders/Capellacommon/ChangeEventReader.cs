...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class ChangeEventReader : XmiElementReader<Auriga.Capellacommon.IChangeEvent>, IXmiElementReader<Auriga.Capellacommon.IChangeEvent>
    {
        ...
        public ChangeEventReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacommon.IChangeEvent Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { if (TryParseEnum<Auriga.Capellacommon.ChangeEventKind>(xmlReader.GetAttribute("kind"), out var parsed)) { poco.Kind = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "Expression", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "features":
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedStateEventRealizations", href); SkipElement(xmlReader); }
                            else { poco.OwnedStateEventRealizations.Add((Auriga.Capellacommon.IStateEventRealization)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ChangeEvent at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
