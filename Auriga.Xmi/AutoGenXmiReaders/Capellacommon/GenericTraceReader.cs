...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class GenericTraceReader : XmiElementReader<Auriga.Capellacommon.IGenericTrace>, IXmiElementReader<Auriga.Capellacommon.IGenericTrace>
    {
        ...
        public GenericTraceReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellacommon.IGenericTrace Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "KeyValuePairs", href); SkipElement(xmlReader); }
                            else { poco.KeyValuePairs.Add((Auriga.Capellacore.IKeyValue)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of GenericTrace at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
