...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class EntryPointPseudoStateReader : XmiElementReader<Auriga.Capellacommon.IEntryPointPseudoState>, IXmiElementReader<Auriga.Capellacommon.IEntryPointPseudoState>
    {
        ...
        public Auriga.Capellacommon.IEntryPointPseudoState Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of EntryPointPseudoState at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
