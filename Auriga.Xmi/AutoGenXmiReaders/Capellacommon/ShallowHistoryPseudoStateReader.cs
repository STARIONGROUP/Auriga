...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class ShallowHistoryPseudoStateReader : XmiElementReader<Auriga.Capellacommon.IShallowHistoryPseudoState>, IXmiElementReader<Auriga.Capellacommon.IShallowHistoryPseudoState>
    {
        ...
        public Auriga.Capellacommon.IShallowHistoryPseudoState Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        break;
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ShallowHistoryPseudoState at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
