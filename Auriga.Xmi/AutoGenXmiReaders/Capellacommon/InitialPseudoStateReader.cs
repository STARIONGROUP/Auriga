...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class InitialPseudoStateReader : XmiElementReader<Auriga.Capellacommon.IInitialPseudoState>, IXmiElementReader<Auriga.Capellacommon.IInitialPseudoState>
    {
        ...
        public Auriga.Capellacommon.IInitialPseudoState Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of InitialPseudoState at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
