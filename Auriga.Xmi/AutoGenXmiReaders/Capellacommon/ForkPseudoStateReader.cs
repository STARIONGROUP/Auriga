...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellacommon
{
    ...
    public class ForkPseudoStateReader : XmiElementReader<Auriga.Capellacommon.IForkPseudoState>, IXmiElementReader<Auriga.Capellacommon.IForkPseudoState>
    {
        ...
        public Auriga.Capellacommon.IForkPseudoState Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        break;
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ForkPseudoState at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
