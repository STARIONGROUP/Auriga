// ------------------------------------------------------------------------------------------------
// <copyright file="FunctionInputPortReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>FunctionInputPort</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class FunctionInputPortReader : XmiElementReader<Auriga.Fa.IFunctionInputPort>, IXmiElementReader<Auriga.Fa.IFunctionInputPort>
    {
        /// <summary>
        ...
        public FunctionInputPortReader(IXmiElementCache cache, IXmiReaderFacade facade, IXmiReaderSettings settings, ILoggerFactory loggerFactory)
            : base(cache, facade, settings, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>FunctionInputPort</c> from the element at the cursor of the supplied reader.
        /// </summary>
        ...
        /// <returns>the populated <see cref="Auriga.Fa.IFunctionInputPort"/></returns>
        public Auriga.Fa.IFunctionInputPort Read(XmlReader xmlReader, string documentName, string namespaceUri)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Fa.FunctionInputPort();

            var xmlLineInfo = xmlReader as IXmlLineInfo;

            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                this.Logger.LogTrace("reading FunctionInputPort at line:position {LineNumber}:{LinePosition}", xmlLineInfo?.LineNumber, xmlLineInfo?.LinePosition);
                ...
                CollectMultiValueReferences(poco, "IncomingExchangeItems", xmlReader.GetAttribute("incomingExchangeItems"));
                CollectSingleValueReference(poco, "InputEvaluationAction", xmlReader.GetAttribute("inputEvaluationAction"));
                {
                    var raw = xmlReader.GetAttribute("isControl");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.IsControl = parsed;
                    }
                }
                {
                    var raw = xmlReader.GetAttribute("isControlType");
                    if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed))
                    {
                        poco.IsControlType = parsed;
                    }
                }
                {
                    if (TryParseEnum<Auriga.Activity.ObjectNodeKind>(xmlReader.GetAttribute("kindOfNode"), out var parsed))
                    {
                ...
                {
                    if (TryParseEnum<Auriga.Activity.ObjectNodeOrderingKind>(xmlReader.GetAttribute("ordering"), out var parsed))
                    {
                    ...
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            case "inState":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "InState", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "incomingExchangeItems":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "IncomingExchangeItems", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "inputEvaluationAction":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "InputEvaluationAction", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "ownedPortAllocations":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedPortAllocations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedPortAllocations.Add((Auriga.Information.IPortAllocation)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedPortRealizations":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedPortRealizations", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedPortRealizations.Add((Auriga.Information.IPortRealization)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }
                            ...
                            case "ownedProtocols":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "OwnedProtocols", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.OwnedProtocols.Add((Auriga.Capellacommon.IStateMachine)this.Facade.QueryElement(xmlReader, documentName, namespaceUri));
                                }

                                break;
                            }
                            case "providedInterfaces":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "ProvidedInterfaces", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "representedComponentPort":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "RepresentedComponentPort", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "requiredInterfaces":
                            {
                                var href = xmlReader.GetAttribute("href");
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectMultiValueReferences(poco, "RequiredInterfaces", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                            ...
                            case "upperBound":
                            {
                                ...
                                if (!string.IsNullOrEmpty(href))
                                {
                                    CollectSingleValueReference(poco, "UpperBound", href);
                                    SkipElement(xmlReader);
                                }
                                else
                                {
                                    poco.UpperBound = (Auriga.Modellingcore.IValueSpecification)this.Facade.QueryElement(xmlReader, documentName, namespaceUri);
                                }
                            ...
                            default:
                                ...
                                {
                                    throw new NotSupportedException($"FunctionInputPortReader: {xmlReader.LocalName} at line:position {xmlLineInfo?.LineNumber}:{xmlLineInfo?.LinePosition}");
                                }

                                this.Logger.LogWarning("Not supported by FunctionInputPortReader: the '{LocalName}' element at line:position {LineNumber}:{LinePosition} is not part of the metamodel and was skipped", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
            ...
            {
                this.Logger.LogWarning("Expected an element to read FunctionInputPort but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
