...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    public class ComponentExchangeAllocationReader : XmiElementReader<Auriga.Fa.IComponentExchangeAllocation>, IXmiElementReader<Auriga.Fa.IComponentExchangeAllocation>
    {
        ...
        public ComponentExchangeAllocationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Fa.IComponentExchangeAllocation Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ComponentExchangeAllocation at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
