...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    public class ComponentFunctionalAllocationReader : XmiElementReader<Auriga.Fa.IComponentFunctionalAllocation>, IXmiElementReader<Auriga.Fa.IComponentFunctionalAllocation>
    {
        ...
        public ComponentFunctionalAllocationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Fa.IComponentFunctionalAllocation Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ComponentFunctionalAllocation at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
