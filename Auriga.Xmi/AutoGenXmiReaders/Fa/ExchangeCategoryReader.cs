...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    public class ExchangeCategoryReader : XmiElementReader<Auriga.Fa.IExchangeCategory>, IXmiElementReader<Auriga.Fa.IExchangeCategory>
    {
        ...
        public ExchangeCategoryReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Fa.IExchangeCategory Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ExchangeCategory at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
