...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    public class ComponentPortAllocationReader : XmiElementReader<Auriga.Fa.IComponentPortAllocation>, IXmiElementReader<Auriga.Fa.IComponentPortAllocation>
    {
        ...
        public ComponentPortAllocationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedComponentPortAllocationEnds", href); SkipElement(xmlReader); }
                            else { poco.OwnedComponentPortAllocationEnds.Add((Auriga.Fa.IComponentPortAllocationEnd)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ComponentPortAllocation at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
