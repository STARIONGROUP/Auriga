...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    public class FunctionalChainReferenceReader : XmiElementReader<Auriga.Fa.IFunctionalChainReference>, IXmiElementReader<Auriga.Fa.IFunctionalChainReference>
    {
        ...
        public FunctionalChainReferenceReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Fa.IFunctionalChainReference Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of FunctionalChainReference at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
