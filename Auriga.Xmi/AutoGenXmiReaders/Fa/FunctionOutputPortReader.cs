...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    public class FunctionOutputPortReader : XmiElementReader<Auriga.Fa.IFunctionOutputPort>, IXmiElementReader<Auriga.Fa.IFunctionOutputPort>
    {
        ...
        public FunctionOutputPortReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OutgoingExchangeItems", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            ...
                            case "upperBound":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of FunctionOutputPort at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
