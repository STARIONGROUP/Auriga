...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    public class FunctionalChainInvolvementFunctionReader : XmiElementReader<Auriga.Fa.IFunctionalChainInvolvementFunction>, IXmiElementReader<Auriga.Fa.IFunctionalChainInvolvementFunction>
    {
        ...
        public FunctionalChainInvolvementFunctionReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Fa.IFunctionalChainInvolvementFunction Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of FunctionalChainInvolvementFunction at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
