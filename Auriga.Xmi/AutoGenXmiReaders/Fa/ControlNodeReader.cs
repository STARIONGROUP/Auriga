...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    public class ControlNodeReader : XmiElementReader<Auriga.Fa.IControlNode>, IXmiElementReader<Auriga.Fa.IControlNode>
    {
        ...
        public ControlNodeReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Fa.IControlNode Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { if (TryParseEnum<Auriga.Fa.ControlNodeKind>(xmlReader.GetAttribute("kind"), out var parsed)) { poco.Kind = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        break;
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ControlNode at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
