...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    /// </summary>
    public class ComponentExchangeReader : XmiElementReader<Auriga.Fa.IComponentExchange>, IXmiElementReader<Auriga.Fa.IComponentExchange>
    {
        /// <summary>
        ...
        public ComponentExchangeReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Fa.IComponentExchange"/></returns>
        public Auriga.Fa.IComponentExchange Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Fa.ComponentExchange();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { if (TryParseEnum<Auriga.Fa.ComponentExchangeKind>(xmlReader.GetAttribute("kind"), out var parsed)) { poco.Kind = parsed; } }
                CollectSingleValueReference(poco, "Link", xmlReader.GetAttribute("link"));
                poco.Name = xmlReader.GetAttribute("name");
                { var raw = xmlReader.GetAttribute("oriented"); if (!string.IsNullOrEmpty(raw) && bool.TryParse(raw, out var parsed)) { poco.Oriented = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "ConvoyedInformations", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "features":
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "Link", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedComponentExchangeEnds", href); SkipElement(xmlReader); }
                            else { poco.OwnedComponentExchangeEnds.Add((Auriga.Fa.IComponentExchangeEnd)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedComponentExchangeFunctionalExchangeAllocations":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedComponentExchangeFunctionalExchangeAllocations", href); SkipElement(xmlReader); }
                            else { poco.OwnedComponentExchangeFunctionalExchangeAllocations.Add((Auriga.Fa.IComponentExchangeFunctionalExchangeAllocation)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "Realizations", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "realizedFlow":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "RealizedFlow", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "source":
                            ...
                            case "target":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ComponentExchange at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
