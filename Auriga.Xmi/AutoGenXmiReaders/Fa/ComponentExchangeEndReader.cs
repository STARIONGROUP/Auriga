...
namespace Auriga.Xmi.AutoGenXmiReaders.Fa
{
    ...
    public class ComponentExchangeEndReader : XmiElementReader<Auriga.Fa.IComponentExchangeEnd>, IXmiElementReader<Auriga.Fa.IComponentExchangeEnd>
    {
        ...
        public ComponentExchangeEndReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Fa.IComponentExchangeEnd Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ComponentExchangeEnd at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
