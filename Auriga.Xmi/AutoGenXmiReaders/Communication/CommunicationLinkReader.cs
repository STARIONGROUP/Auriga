...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Communication
{
    ...
    public class CommunicationLinkReader : XmiElementReader<Auriga.Information.Communication.ICommunicationLink>, IXmiElementReader<Auriga.Information.Communication.ICommunicationLink>
    {
        ...
        public CommunicationLinkReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Information.Communication.ICommunicationLink Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { if (TryParseEnum<Auriga.Information.Communication.CommunicationLinkKind>(xmlReader.GetAttribute("kind"), out var parsed)) { poco.Kind = parsed; } }
                { if (TryParseEnum<Auriga.Information.Communication.CommunicationLinkProtocol>(xmlReader.GetAttribute("protocol"), out var parsed)) { poco.Protocol = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "ExchangeItem", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "features":
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CommunicationLink at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
