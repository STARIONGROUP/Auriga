...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Communication
{
    ...
    /// </summary>
    public class MessageReader : XmiElementReader<Auriga.Information.Communication.IMessage>, IXmiElementReader<Auriga.Information.Communication.IMessage>
    {
        /// <summary>
        ...
        public MessageReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Information.Communication.IMessage"/></returns>
        public Auriga.Information.Communication.IMessage Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Information.Communication.Message();
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Message at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
