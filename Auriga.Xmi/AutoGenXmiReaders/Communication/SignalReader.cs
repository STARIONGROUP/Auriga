...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Communication
{
    ...
    public class SignalReader : XmiElementReader<Auriga.Information.Communication.ISignal>, IXmiElementReader<Auriga.Information.Communication.ISignal>
    {
        ...
        public SignalReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Information.Communication.ISignal Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "SignalInstances", href); SkipElement(xmlReader); }
                            else { poco.SignalInstances.Add((Auriga.Information.Communication.ISignalInstance)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Signal at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
