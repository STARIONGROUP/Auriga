...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Communication
{
    ...
    public class ExceptionReader : XmiElementReader<Auriga.Information.Communication.IException>, IXmiElementReader<Auriga.Information.Communication.IException>
    {
        ...
        public ExceptionReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Exception at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
