...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Communication
{
    ...
    /// </summary>
    public class SignalInstanceReader : XmiElementReader<Auriga.Information.Communication.ISignalInstance>, IXmiElementReader<Auriga.Information.Communication.ISignalInstance>
    {
        /// <summary>
        ...
        public SignalInstanceReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Information.Communication.ISignalInstance"/></returns>
        public Auriga.Information.Communication.ISignalInstance Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Information.Communication.SignalInstance();
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of SignalInstance at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
