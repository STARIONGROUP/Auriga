...
namespace Auriga.Xmi.AutoGenXmiReaders.Capellamodeller
{
    ...
    public class SystemEngineeringPkgReader : XmiElementReader<Auriga.Capellamodeller.ISystemEngineeringPkg>, IXmiElementReader<Auriga.Capellamodeller.ISystemEngineeringPkg>
    {
        ...
        public SystemEngineeringPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Capellamodeller.ISystemEngineeringPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedSystemEngineerings", href); SkipElement(xmlReader); }
                            else { poco.OwnedSystemEngineerings.Add((Auriga.Capellamodeller.ISystemEngineering)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedTraces":
                        {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of SystemEngineeringPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
