                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "ownedDefinitions":
                        {
                            ...
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedDefinitions", href); SkipElement(xmlReader); }
                            else { poco.OwnedDefinitions.Add((Auriga.Re.ICompliancyDefinition)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedExtensions":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CompliancyDefinitionPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
