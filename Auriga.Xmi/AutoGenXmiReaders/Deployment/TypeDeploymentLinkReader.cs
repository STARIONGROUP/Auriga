...
namespace Auriga.Xmi.AutoGenXmiReaders.Pa.Deployment
{
    ...
    public class TypeDeploymentLinkReader : XmiElementReader<Auriga.Pa.Deployment.ITypeDeploymentLink>, IXmiElementReader<Auriga.Pa.Deployment.ITypeDeploymentLink>
    {
        ...
        public TypeDeploymentLinkReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of TypeDeploymentLink at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
