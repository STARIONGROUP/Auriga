...
namespace Auriga.Xmi.AutoGenXmiReaders.Pa.Deployment
{
    ...
    public class DeploymentAspectReader : XmiElementReader<Auriga.Pa.Deployment.IDeploymentAspect>, IXmiElementReader<Auriga.Pa.Deployment.IDeploymentAspect>
    {
        ...
        public DeploymentAspectReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Pa.Deployment.IDeploymentAspect Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedConfigurations", href); SkipElement(xmlReader); }
                            else { poco.OwnedConfigurations.Add((Auriga.Pa.Deployment.IDeploymentConfiguration)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedDeploymentAspects", href); SkipElement(xmlReader); }
                            else { poco.OwnedDeploymentAspects.Add((Auriga.Pa.Deployment.IDeploymentAspect)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of DeploymentAspect at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
