...
namespace Auriga.Xmi.AutoGenXmiReaders.Pa.Deployment
{
    ...
    public class ConnectionInstanceReader : XmiElementReader<Auriga.Pa.Deployment.IConnectionInstance>, IXmiElementReader<Auriga.Pa.Deployment.IConnectionInstance>
    {
        ...
        public ConnectionInstanceReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Pa.Deployment.IConnectionInstance Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "ConnectionEnds", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "features":
                            ...
                            case "type":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ConnectionInstance at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
