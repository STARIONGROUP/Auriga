...
namespace Auriga.Xmi.AutoGenXmiReaders.Pa.Deployment
{
    ...
    public class DeploymentConfigurationReader : XmiElementReader<Auriga.Pa.Deployment.IDeploymentConfiguration>, IXmiElementReader<Auriga.Pa.Deployment.IDeploymentConfiguration>
    {
        ...
        public DeploymentConfigurationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Pa.Deployment.IDeploymentConfiguration Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedPhysicalInstances", href); SkipElement(xmlReader); }
                            else { poco.OwnedPhysicalInstances.Add((Auriga.Pa.Deployment.IAbstractPhysicalInstance)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of DeploymentConfiguration at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
