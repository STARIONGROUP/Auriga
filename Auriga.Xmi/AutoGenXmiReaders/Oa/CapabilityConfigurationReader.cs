...
namespace Auriga.Xmi.AutoGenXmiReaders.Oa
{
    ...
    public class CapabilityConfigurationReader : XmiElementReader<Auriga.Oa.ICapabilityConfiguration>, IXmiElementReader<Auriga.Oa.ICapabilityConfiguration>
    {
        ...
        public CapabilityConfigurationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "ConfiguredCapability", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "features":
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CapabilityConfiguration at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
