...
namespace Auriga.Xmi.AutoGenXmiReaders.Oa
{
    ...
    /// </summary>
    public class LocationReader : XmiElementReader<Auriga.Oa.ILocation>, IXmiElementReader<Auriga.Oa.ILocation>
    {
        /// <summary>
        ...
        public LocationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Oa.ILocation"/></returns>
        public Auriga.Oa.ILocation Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Oa.Location();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "LocatedEntities", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "namingRules":
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Location at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
