...
namespace Auriga.Xmi.AutoGenXmiReaders.Oa
{
    ...
    public class ConceptPkgReader : XmiElementReader<Auriga.Oa.IConceptPkg>, IXmiElementReader<Auriga.Oa.IConceptPkg>
    {
        ...
        public ConceptPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Oa.IConceptPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedConceptPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedConceptPkgs.Add((Auriga.Oa.IConceptPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedConcepts":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedConcepts", href); SkipElement(xmlReader); }
                            else { poco.OwnedConcepts.Add((Auriga.Oa.IConcept)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ConceptPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
