// ------------------------------------------------------------------------------------------------
// <copyright file="EntityPkgReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.AutoGenXmiReaders.Oa
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>EntityPkg</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class EntityPkgReader : XmiElementReader<Auriga.Oa.IEntityPkg>, IXmiElementReader<Auriga.Oa.IEntityPkg>
    {
        /// <summary>
        ...
        public EntityPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
            : base(cache, facade, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>EntityPkg</c> from the element at the cursor of the supplied reader.
        /// </summary>
        /// <param name="xmlReader">the reader positioned on the element</param>
        /// <returns>the populated <see cref="Auriga.Oa.IEntityPkg"/></returns>
        public Auriga.Oa.IEntityPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Oa.EntityPkg();
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "ownedEntities":
                        {
                            ...
                            case "ownedEntityPkgs":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedEntityPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedEntityPkgs.Add((Auriga.Oa.IEntityPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedLocations", href); SkipElement(xmlReader); }
                            else { poco.OwnedLocations.Add((Auriga.Oa.ILocation)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedParts", href); SkipElement(xmlReader); }
                            else { poco.OwnedParts.Add((Auriga.Cs.IPart)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of EntityPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            ...
            {
                this.Logger.LogWarning("Expected an element to read EntityPkg but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
