            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedOperationalActivities", href); SkipElement(xmlReader); }
                            else { poco.OwnedOperationalActivities.Add((Auriga.Oa.IOperationalActivity)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
