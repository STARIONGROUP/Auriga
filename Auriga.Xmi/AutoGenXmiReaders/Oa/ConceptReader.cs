...
namespace Auriga.Xmi.AutoGenXmiReaders.Oa
{
    ...
    public class ConceptReader : XmiElementReader<Auriga.Oa.IConcept>, IXmiElementReader<Auriga.Oa.IConcept>
    {
        ...
        public ConceptReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Oa.IConcept Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "CompositeLinks", href); SkipElement(xmlReader); }
                            else { poco.CompositeLinks.Add((Auriga.Oa.IItemInConcept)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "features":
                        {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Concept at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
