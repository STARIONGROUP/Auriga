...
namespace Auriga.Xmi.AutoGenXmiReaders.Oa
{
    ...
    public class OperationalCapabilityPkgReader : XmiElementReader<Auriga.Oa.IOperationalCapabilityPkg>, IXmiElementReader<Auriga.Oa.IOperationalCapabilityPkg>
    {
        ...
        public OperationalCapabilityPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Oa.IOperationalCapabilityPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedCapabilityConfigurations", href); SkipElement(xmlReader); }
                            else { poco.OwnedCapabilityConfigurations.Add((Auriga.Oa.ICapabilityConfiguration)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedConceptCompliances":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedConceptCompliances", href); SkipElement(xmlReader); }
                            else { poco.OwnedConceptCompliances.Add((Auriga.Oa.IConceptCompliance)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedOperationalCapabilities", href); SkipElement(xmlReader); }
                            else { poco.OwnedOperationalCapabilities.Add((Auriga.Oa.IOperationalCapability)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedOperationalCapabilityPkgs":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedOperationalCapabilityPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedOperationalCapabilityPkgs.Add((Auriga.Oa.IOperationalCapabilityPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of OperationalCapabilityPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
