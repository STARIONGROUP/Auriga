...
namespace Auriga.Xmi.AutoGenXmiReaders.Oa
{
    ...
    public class CommunityOfInterestReader : XmiElementReader<Auriga.Oa.ICommunityOfInterest>, IXmiElementReader<Auriga.Oa.ICommunityOfInterest>
    {
        ...
        public CommunityOfInterestReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Oa.ICommunityOfInterest Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "CommunityOfInterestCompositions", href); SkipElement(xmlReader); }
                            else { poco.CommunityOfInterestCompositions.Add((Auriga.Oa.ICommunityOfInterestComposition)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "features":
                        {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CommunityOfInterest at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
