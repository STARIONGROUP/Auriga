...
namespace Auriga.Xmi.AutoGenXmiReaders.Oa
{
    ...
    public class OrganisationalUnitReader : XmiElementReader<Auriga.Oa.IOrganisationalUnit>, IXmiElementReader<Auriga.Oa.IOrganisationalUnit>
    {
        ...
        public OrganisationalUnitReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Oa.IOrganisationalUnit Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "CommunityOfInterestMemberships", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "features":
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OrganisationalUnitCompositions", href); SkipElement(xmlReader); }
                            else { poco.OrganisationalUnitCompositions.Add((Auriga.Oa.IOrganisationalUnitComposition)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of OrganisationalUnit at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
