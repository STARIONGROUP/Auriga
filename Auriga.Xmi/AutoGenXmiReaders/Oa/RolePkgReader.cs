...
namespace Auriga.Xmi.AutoGenXmiReaders.Oa
{
    ...
    public class RolePkgReader : XmiElementReader<Auriga.Oa.IRolePkg>, IXmiElementReader<Auriga.Oa.IRolePkg>
    {
        ...
        public RolePkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Oa.IRolePkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedRolePkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedRolePkgs.Add((Auriga.Oa.IRolePkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedRoles":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedRoles", href); SkipElement(xmlReader); }
                            else { poco.OwnedRoles.Add((Auriga.Oa.IRole)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedTraces":
                        {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of RolePkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
