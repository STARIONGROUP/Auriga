...
namespace Auriga.Xmi.AutoGenXmiReaders.La
{
    ...
    /// </summary>
    public class LogicalFunctionPkgReader : XmiElementReader<Auriga.La.ILogicalFunctionPkg>, IXmiElementReader<Auriga.La.ILogicalFunctionPkg>
    {
        /// <summary>
        ...
        public LogicalFunctionPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.La.ILogicalFunctionPkg"/></returns>
        public Auriga.La.ILogicalFunctionPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.La.LogicalFunctionPkg();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedLogicalFunctions", href); SkipElement(xmlReader); }
                            else { poco.OwnedLogicalFunctions.Add((Auriga.La.ILogicalFunction)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of LogicalFunctionPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
