...
namespace Auriga.Xmi.AutoGenXmiReaders.La
{
    ...
    public class LogicalArchitecturePkgReader : XmiElementReader<Auriga.La.ILogicalArchitecturePkg>, IXmiElementReader<Auriga.La.ILogicalArchitecturePkg>
    {
        ...
        public LogicalArchitecturePkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.La.ILogicalArchitecturePkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of LogicalArchitecturePkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
