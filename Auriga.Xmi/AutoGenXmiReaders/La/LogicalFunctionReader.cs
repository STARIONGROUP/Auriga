            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedLogicalFunctionPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedLogicalFunctionPkgs.Add((Auriga.La.ILogicalFunctionPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedMaxCard":
                        {
