// ------------------------------------------------------------------------------------------------
// <copyright file="LogicalComponentPkgReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.AutoGenXmiReaders.La
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>LogicalComponentPkg</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class LogicalComponentPkgReader : XmiElementReader<Auriga.La.ILogicalComponentPkg>, IXmiElementReader<Auriga.La.ILogicalComponentPkg>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="LogicalComponentPkgReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public LogicalComponentPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
            : base(cache, facade, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>LogicalComponentPkg</c> from the element at the cursor of the supplied reader.
        /// </summary>
        /// <param name="xmlReader">the reader positioned on the element</param>
        /// <returns>the populated <see cref="Auriga.La.ILogicalComponentPkg"/></returns>
        public Auriga.La.ILogicalComponentPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.La.LogicalComponentPkg();
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of LogicalComponentPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            ...
            {
                this.Logger.LogWarning("Expected an element to read LogicalComponentPkg but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
