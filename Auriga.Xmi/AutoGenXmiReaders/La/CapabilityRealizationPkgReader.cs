...
namespace Auriga.Xmi.AutoGenXmiReaders.La
{
    ...
    public class CapabilityRealizationPkgReader : XmiElementReader<Auriga.La.ICapabilityRealizationPkg>, IXmiElementReader<Auriga.La.ICapabilityRealizationPkg>
    {
        ...
        public CapabilityRealizationPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.La.ICapabilityRealizationPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedCapabilityRealizationPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedCapabilityRealizationPkgs.Add((Auriga.La.ICapabilityRealizationPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedCapabilityRealizations":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedCapabilityRealizations", href); SkipElement(xmlReader); }
                            else { poco.OwnedCapabilityRealizations.Add((Auriga.La.ICapabilityRealization)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CapabilityRealizationPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
