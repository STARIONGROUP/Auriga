...
namespace Auriga.Xmi.AutoGenXmiReaders.La
{
    ...
    public class SystemAnalysisRealizationReader : XmiElementReader<Auriga.La.ISystemAnalysisRealization>, IXmiElementReader<Auriga.La.ISystemAnalysisRealization>
    {
        ...
        public SystemAnalysisRealizationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.La.ISystemAnalysisRealization Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of SystemAnalysisRealization at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
