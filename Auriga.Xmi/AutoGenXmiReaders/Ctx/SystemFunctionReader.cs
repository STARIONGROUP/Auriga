// ------------------------------------------------------------------------------------------------
// <copyright file="SystemFunctionReader.cs" company="Starion Group S.A.">
//
...
namespace Auriga.Xmi.AutoGenXmiReaders.Ctx
{
    ...
    /// <summary>
    /// The generated XMI reader that instantiates and populates an <c>SystemFunction</c> from its
    /// XMI representation. Contained elements are read recursively; cross-references are collected for the
    /// reference-resolution pass.
    /// </summary>
    public class SystemFunctionReader : XmiElementReader<Auriga.Ctx.ISystemFunction>, IXmiElementReader<Auriga.Ctx.ISystemFunction>
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="SystemFunctionReader"/> class.
        /// </summary>
        /// <param name="cache">the element cache</param>
        /// <param name="facade">the reader facade used to read contained elements</param>
        /// <param name="loggerFactory">the logger factory, or <c>null</c> to disable logging</param>
        public SystemFunctionReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
            : base(cache, facade, loggerFactory)
        {
        }

        /// <summary>
        /// Reads an <c>SystemFunction</c> from the element at the cursor of the supplied reader.
        /// </summary>
        /// <param name="xmlReader">the reader positioned on the element</param>
        /// <returns>the populated <see cref="Auriga.Ctx.ISystemFunction"/></returns>
        public Auriga.Ctx.ISystemFunction Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Ctx.SystemFunction();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "Arguments", href); SkipElement(xmlReader); }
                            else { poco.Arguments.Add((Auriga.Activity.IInputPin)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "Behavior", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "context":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "Context", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "features":
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "Inputs", href); SkipElement(xmlReader); }
                            else { poco.Inputs.Add((Auriga.Activity.IInputPin)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "localPostcondition":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "LocalPostcondition", href); SkipElement(xmlReader); }
                            else { var contained = (Auriga.Modellingcore.IAbstractConstraint)this.Facade.QueryElement(xmlReader); contained.Container = poco; poco.LocalPostcondition = contained; }
                            break;
                        }
                            case "localPrecondition":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "LocalPrecondition", href); SkipElement(xmlReader); }
                            else { var contained = (Auriga.Modellingcore.IAbstractConstraint)this.Facade.QueryElement(xmlReader); contained.Container = poco; poco.LocalPrecondition = contained; }
                            break;
                        }
                            case "namingRules":
                        {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "Outputs", href); SkipElement(xmlReader); }
                            else { poco.Outputs.Add((Auriga.Activity.IOutputPin)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedFunctionRealizations", href); SkipElement(xmlReader); }
                            else { poco.OwnedFunctionRealizations.Add((Auriga.Fa.IFunctionRealization)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedFunctionalExchanges", href); SkipElement(xmlReader); }
                            else { poco.OwnedFunctionalExchanges.Add((Auriga.Fa.IFunctionalExchange)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedFunctions":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedFunctions", href); SkipElement(xmlReader); }
                            else { poco.OwnedFunctions.Add((Auriga.Fa.IAbstractFunction)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedHandlers":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedHandlers", href); SkipElement(xmlReader); }
                            else { poco.OwnedHandlers.Add((Auriga.Activity.IExceptionHandler)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedMaxCard":
                        {
                            ...
                            case "ownedPropertyValues":
                        {
                            ...
                            case "ownedSystemFunctionPkgs":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedSystemFunctionPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedSystemFunctionPkgs.Add((Auriga.Ctx.ISystemFunctionPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedTraces":
                        {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "Results", href); SkipElement(xmlReader); }
                            else { poco.Results.Add((Auriga.Activity.IOutputPin)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of SystemFunction at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            ...
            {
                this.Logger.LogWarning("Expected an element to read SystemFunction but found {NodeType} at line {Line}:{Position}", xmlReader.NodeType, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
            }
