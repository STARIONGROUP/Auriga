...
namespace Auriga.Xmi.AutoGenXmiReaders.Ctx
{
    ...
    public class MissionInvolvementReader : XmiElementReader<Auriga.Ctx.IMissionInvolvement>, IXmiElementReader<Auriga.Ctx.IMissionInvolvement>
    {
        ...
        public MissionInvolvementReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Ctx.IMissionInvolvement Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of MissionInvolvement at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
