...
namespace Auriga.Xmi.AutoGenXmiReaders.Ctx
{
    ...
    public class SystemCommunicationReader : XmiElementReader<Auriga.Ctx.ISystemCommunication>, IXmiElementReader<Auriga.Ctx.ISystemCommunication>
    {
        ...
        public SystemCommunicationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Ctx.ISystemCommunication Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "Ends", href); SkipElement(xmlReader); }
                            else { poco.Ends.Add((Auriga.Ctx.ISystemCommunicationHook)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "features":
                        {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of SystemCommunication at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
