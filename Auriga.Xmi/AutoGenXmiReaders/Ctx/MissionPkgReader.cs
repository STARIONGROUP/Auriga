...
namespace Auriga.Xmi.AutoGenXmiReaders.Ctx
{
    ...
    public class MissionPkgReader : XmiElementReader<Auriga.Ctx.IMissionPkg>, IXmiElementReader<Auriga.Ctx.IMissionPkg>
    {
        ...
        public MissionPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Ctx.IMissionPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedMissionPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedMissionPkgs.Add((Auriga.Ctx.IMissionPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedMissions":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedMissions", href); SkipElement(xmlReader); }
                            else { poco.OwnedMissions.Add((Auriga.Ctx.IMission)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of MissionPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
