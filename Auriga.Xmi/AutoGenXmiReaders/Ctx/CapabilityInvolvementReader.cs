...
namespace Auriga.Xmi.AutoGenXmiReaders.Ctx
{
    ...
    public class CapabilityInvolvementReader : XmiElementReader<Auriga.Ctx.ICapabilityInvolvement>, IXmiElementReader<Auriga.Ctx.ICapabilityInvolvement>
    {
        ...
        public CapabilityInvolvementReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Ctx.ICapabilityInvolvement Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CapabilityInvolvement at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
