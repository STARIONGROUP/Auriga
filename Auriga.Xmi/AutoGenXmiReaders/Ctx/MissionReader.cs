...
namespace Auriga.Xmi.AutoGenXmiReaders.Ctx
{
    ...
    public class MissionReader : XmiElementReader<Auriga.Ctx.IMission>, IXmiElementReader<Auriga.Ctx.IMission>
    {
        ...
        public MissionReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Ctx.IMission Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedCapabilityExploitations", href); SkipElement(xmlReader); }
                            else { poco.OwnedCapabilityExploitations.Add((Auriga.Ctx.ICapabilityExploitation)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedMissionInvolvements", href); SkipElement(xmlReader); }
                            else { poco.OwnedMissionInvolvements.Add((Auriga.Ctx.IMissionInvolvement)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Mission at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
