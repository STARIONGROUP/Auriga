...
namespace Auriga.Xmi.AutoGenXmiReaders.Ctx
{
    ...
    public class SystemFunctionPkgReader : XmiElementReader<Auriga.Ctx.ISystemFunctionPkg>, IXmiElementReader<Auriga.Ctx.ISystemFunctionPkg>
    {
        ...
        public SystemFunctionPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Ctx.ISystemFunctionPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedSystemFunctions", href); SkipElement(xmlReader); }
                            else { poco.OwnedSystemFunctions.Add((Auriga.Ctx.ISystemFunction)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedTraces":
                        {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of SystemFunctionPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
