...
namespace Auriga.Xmi.AutoGenXmiReaders.Ctx
{
    ...
    public class CapabilityPkgReader : XmiElementReader<Auriga.Ctx.ICapabilityPkg>, IXmiElementReader<Auriga.Ctx.ICapabilityPkg>
    {
        ...
        public CapabilityPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Ctx.ICapabilityPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedCapabilities", href); SkipElement(xmlReader); }
                            else { poco.OwnedCapabilities.Add((Auriga.Ctx.ICapability)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "ownedCapabilityPkgs":
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "OwnedCapabilityPkgs", href); SkipElement(xmlReader); }
                            else { poco.OwnedCapabilityPkgs.Add((Auriga.Ctx.ICapabilityPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CapabilityPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
