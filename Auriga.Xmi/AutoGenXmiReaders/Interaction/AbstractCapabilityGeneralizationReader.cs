...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class AbstractCapabilityGeneralizationReader : XmiElementReader<Auriga.Interaction.IAbstractCapabilityGeneralization>, IXmiElementReader<Auriga.Interaction.IAbstractCapabilityGeneralization>
    {
        ...
        public AbstractCapabilityGeneralizationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IAbstractCapabilityGeneralization Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "Super", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of AbstractCapabilityGeneralization at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
