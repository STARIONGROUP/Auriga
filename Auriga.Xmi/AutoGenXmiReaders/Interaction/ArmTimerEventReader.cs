...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class ArmTimerEventReader : XmiElementReader<Auriga.Interaction.IArmTimerEvent>, IXmiElementReader<Auriga.Interaction.IArmTimerEvent>
    {
        ...
        public Auriga.Interaction.IArmTimerEvent Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ArmTimerEvent at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
