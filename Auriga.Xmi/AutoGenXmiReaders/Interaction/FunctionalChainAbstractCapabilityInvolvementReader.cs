...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class FunctionalChainAbstractCapabilityInvolvementReader : XmiElementReader<Auriga.Interaction.IFunctionalChainAbstractCapabilityInvolvement>, IXmiElementReader<Auriga.Interaction.IFunctionalChainAbstractCapabilityInvolvement>
    {
        ...
        public FunctionalChainAbstractCapabilityInvolvementReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IFunctionalChainAbstractCapabilityInvolvement Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of FunctionalChainAbstractCapabilityInvolvement at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
