...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class CancelTimerEventReader : XmiElementReader<Auriga.Interaction.ICancelTimerEvent>, IXmiElementReader<Auriga.Interaction.ICancelTimerEvent>
    {
        ...
        public Auriga.Interaction.ICancelTimerEvent Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CancelTimerEvent at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
