...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    /// </summary>
    public class InteractionOperandReader : XmiElementReader<Auriga.Interaction.IInteractionOperand>, IXmiElementReader<Auriga.Interaction.IInteractionOperand>
    {
        /// <summary>
        ...
        public InteractionOperandReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Interaction.IInteractionOperand"/></returns>
        public Auriga.Interaction.IInteractionOperand Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Interaction.InteractionOperand();
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "ReferencedInteractionFragments", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of InteractionOperand at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
