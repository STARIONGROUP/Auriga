...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class ConstraintDurationReader : XmiElementReader<Auriga.Interaction.IConstraintDuration>, IXmiElementReader<Auriga.Interaction.IConstraintDuration>
    {
        ...
        public ConstraintDurationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IConstraintDuration Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ConstraintDuration at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
