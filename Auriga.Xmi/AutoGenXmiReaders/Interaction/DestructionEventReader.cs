...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class DestructionEventReader : XmiElementReader<Auriga.Interaction.IDestructionEvent>, IXmiElementReader<Auriga.Interaction.IDestructionEvent>
    {
        ...
        public Auriga.Interaction.IDestructionEvent Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of DestructionEvent at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
