...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class EventReceiptOperationReader : XmiElementReader<Auriga.Interaction.IEventReceiptOperation>, IXmiElementReader<Auriga.Interaction.IEventReceiptOperation>
    {
        ...
        public EventReceiptOperationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IEventReceiptOperation Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of EventReceiptOperation at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
