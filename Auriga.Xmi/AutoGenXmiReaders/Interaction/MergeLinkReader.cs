...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class MergeLinkReader : XmiElementReader<Auriga.Interaction.IMergeLink>, IXmiElementReader<Auriga.Interaction.IMergeLink>
    {
        ...
        public MergeLinkReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IMergeLink Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of MergeLink at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
