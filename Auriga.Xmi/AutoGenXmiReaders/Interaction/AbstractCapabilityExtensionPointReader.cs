...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class AbstractCapabilityExtensionPointReader : XmiElementReader<Auriga.Interaction.IAbstractCapabilityExtensionPoint>, IXmiElementReader<Auriga.Interaction.IAbstractCapabilityExtensionPoint>
    {
        ...
        public AbstractCapabilityExtensionPointReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IAbstractCapabilityExtensionPoint Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "ExtendLinks", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "features":
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of AbstractCapabilityExtensionPoint at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
