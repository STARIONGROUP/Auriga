...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class CreationEventReader : XmiElementReader<Auriga.Interaction.ICreationEvent>, IXmiElementReader<Auriga.Interaction.ICreationEvent>
    {
        ...
        public Auriga.Interaction.ICreationEvent Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CreationEvent at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
