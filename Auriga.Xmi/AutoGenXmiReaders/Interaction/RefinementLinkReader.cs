...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class RefinementLinkReader : XmiElementReader<Auriga.Interaction.IRefinementLink>, IXmiElementReader<Auriga.Interaction.IRefinementLink>
    {
        ...
        public RefinementLinkReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IRefinementLink Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of RefinementLink at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
