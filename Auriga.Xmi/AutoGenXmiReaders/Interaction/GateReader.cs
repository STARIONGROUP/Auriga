...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class GateReader : XmiElementReader<Auriga.Interaction.IGate>, IXmiElementReader<Auriga.Interaction.IGate>
    {
        ...
        public GateReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IGate Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Gate at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
