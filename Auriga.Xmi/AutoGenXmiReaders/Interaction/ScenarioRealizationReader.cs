...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class ScenarioRealizationReader : XmiElementReader<Auriga.Interaction.IScenarioRealization>, IXmiElementReader<Auriga.Interaction.IScenarioRealization>
    {
        ...
        public ScenarioRealizationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IScenarioRealization Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ScenarioRealization at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
