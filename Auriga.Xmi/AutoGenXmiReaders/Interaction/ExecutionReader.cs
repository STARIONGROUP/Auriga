...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class ExecutionReader : XmiElementReader<Auriga.Interaction.IExecution>, IXmiElementReader<Auriga.Interaction.IExecution>
    {
        ...
        public ExecutionReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IExecution Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Execution at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
