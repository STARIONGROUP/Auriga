...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class CombinedFragmentReader : XmiElementReader<Auriga.Interaction.ICombinedFragment>, IXmiElementReader<Auriga.Interaction.ICombinedFragment>
    {
        ...
        public CombinedFragmentReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.ICombinedFragment Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { if (TryParseEnum<Auriga.Interaction.InteractionOperatorKind>(xmlReader.GetAttribute("operator"), out var parsed)) { poco.Operator = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "ReferencedOperands", href); SkipElement(xmlReader); }
                            else { SkipElement(xmlReader); }
                            break;
                        }
                            case "start":
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CombinedFragment at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
