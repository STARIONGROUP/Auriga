...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class ExecutionEventReader : XmiElementReader<Auriga.Interaction.IExecutionEvent>, IXmiElementReader<Auriga.Interaction.IExecutionEvent>
    {
        ...
        public Auriga.Interaction.IExecutionEvent Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ExecutionEvent at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
