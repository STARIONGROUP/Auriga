...
namespace Auriga.Xmi.AutoGenXmiReaders.Interaction
{
    ...
    public class ExecutionEndReader : XmiElementReader<Auriga.Interaction.IExecutionEnd>, IXmiElementReader<Auriga.Interaction.IExecutionEnd>
    {
        ...
        public ExecutionEndReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Interaction.IExecutionEnd Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ExecutionEnd at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
