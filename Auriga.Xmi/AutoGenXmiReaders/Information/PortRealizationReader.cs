...
namespace Auriga.Xmi.AutoGenXmiReaders.Information
{
    ...
    public class PortRealizationReader : XmiElementReader<Auriga.Information.IPortRealization>, IXmiElementReader<Auriga.Information.IPortRealization>
    {
        ...
        public PortRealizationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Information.IPortRealization Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PortRealization at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
