...
namespace Auriga.Xmi.AutoGenXmiReaders.Information
{
    ...
    public class PortAllocationReader : XmiElementReader<Auriga.Information.IPortAllocation>, IXmiElementReader<Auriga.Information.IPortAllocation>
    {
        ...
        public PortAllocationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Information.IPortAllocation Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of PortAllocation at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
