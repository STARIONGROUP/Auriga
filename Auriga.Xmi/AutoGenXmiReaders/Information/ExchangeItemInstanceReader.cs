...
namespace Auriga.Xmi.AutoGenXmiReaders.Information
{
    ...
    public class ExchangeItemInstanceReader : XmiElementReader<Auriga.Information.IExchangeItemInstance>, IXmiElementReader<Auriga.Information.IExchangeItemInstance>
    {
        ...
        public Auriga.Information.IExchangeItemInstance Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of ExchangeItemInstance at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
