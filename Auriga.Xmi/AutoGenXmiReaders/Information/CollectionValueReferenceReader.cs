...
namespace Auriga.Xmi.AutoGenXmiReaders.Information
{
    ...
    public class CollectionValueReferenceReader : XmiElementReader<Auriga.Information.ICollectionValueReference>, IXmiElementReader<Auriga.Information.ICollectionValueReference>
    {
        ...
        public CollectionValueReferenceReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Information.ICollectionValueReference Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of CollectionValueReference at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
