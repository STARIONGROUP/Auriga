...
namespace Auriga.Xmi.AutoGenXmiReaders.Information
{
    ...
    public class UnitReader : XmiElementReader<Auriga.Information.IUnit>, IXmiElementReader<Auriga.Information.IUnit>
    {
        ...
        public Auriga.Information.IUnit Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        switch (xmlReader.LocalName)
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of Unit at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
                                SkipElement(xmlReader);
                                break;
                        }
                    }
