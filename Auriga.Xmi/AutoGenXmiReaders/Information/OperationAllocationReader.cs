...
namespace Auriga.Xmi.AutoGenXmiReaders.Information
{
    ...
    public class OperationAllocationReader : XmiElementReader<Auriga.Information.IOperationAllocation>, IXmiElementReader<Auriga.Information.IOperationAllocation>
    {
        ...
        public OperationAllocationReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Information.IOperationAllocation Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "targetElement":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of OperationAllocation at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
