...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Datavalue
{
    ...
    public class EnumerationLiteralReader : XmiElementReader<Auriga.Information.Datavalue.IEnumerationLiteral>, IXmiElementReader<Auriga.Information.Datavalue.IEnumerationLiteral>
    {
        ...
        public EnumerationLiteralReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Information.Datavalue.IEnumerationLiteral Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "DomainValue", href); SkipElement(xmlReader); }
                            else { var contained = (Auriga.Information.Datavalue.IDataValue)this.Facade.QueryElement(xmlReader); contained.Container = poco; poco.DomainValue = contained; }
                            break;
                        }
                            case "features":
                        {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of EnumerationLiteral at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
