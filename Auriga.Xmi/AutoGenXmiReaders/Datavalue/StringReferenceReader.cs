...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Datavalue
{
    ...
    /// </summary>
    public class StringReferenceReader : XmiElementReader<Auriga.Information.Datavalue.IStringReference>, IXmiElementReader<Auriga.Information.Datavalue.IStringReference>
    {
        /// <summary>
        ...
        public StringReferenceReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        /// <returns>the populated <see cref="Auriga.Information.Datavalue.IStringReference"/></returns>
        public Auriga.Information.Datavalue.IStringReference Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            var poco = new Auriga.Information.Datavalue.StringReference();
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of StringReference at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
