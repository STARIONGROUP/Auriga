...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Datavalue
{
    ...
    public class LiteralNumericValueReader : XmiElementReader<Auriga.Information.Datavalue.ILiteralNumericValue>, IXmiElementReader<Auriga.Information.Datavalue.ILiteralNumericValue>
    {
        ...
        public Auriga.Information.Datavalue.ILiteralNumericValue Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        break;
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of LiteralNumericValue at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
