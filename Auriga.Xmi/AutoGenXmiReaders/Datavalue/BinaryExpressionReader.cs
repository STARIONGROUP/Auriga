    ...
    public class BinaryExpressionReader : XmiElementReader<Auriga.Information.Datavalue.IBinaryExpression>, IXmiElementReader<Auriga.Information.Datavalue.IBinaryExpression>
    {
        ...
        public Auriga.Information.Datavalue.IBinaryExpression Read(XmlReader xmlReader)
        {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                { if (TryParseEnum<Auriga.Information.Datavalue.BinaryOperator>(xmlReader.GetAttribute("operator"), out var parsed)) { poco.Operator = parsed; } }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "OwnedLeftOperand", href); SkipElement(xmlReader); }
                            else { var contained = (Auriga.Information.Datavalue.IDataValue)this.Facade.QueryElement(xmlReader); contained.Container = poco; poco.OwnedLeftOperand = contained; }
                            break;
                        }
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "OwnedRightOperand", href); SkipElement(xmlReader); }
                            else { var contained = (Auriga.Information.Datavalue.IDataValue)this.Facade.QueryElement(xmlReader); contained.Container = poco; poco.OwnedRightOperand = contained; }
                            break;
                        }
                            case "status":
                        {
                            ...
                            case "unit":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of BinaryExpression at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
