...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Datavalue
{
    ...
    public class NumericReferenceReader : XmiElementReader<Auriga.Information.Datavalue.INumericReference>, IXmiElementReader<Auriga.Information.Datavalue.INumericReference>
    {
        ...
        public NumericReferenceReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Information.Datavalue.INumericReference Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                            ...
                            case "unit":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of NumericReference at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
