...
namespace Auriga.Xmi.AutoGenXmiReaders.Information.Datavalue
{
    ...
    public class OpaqueExpressionReader : XmiElementReader<Auriga.Information.Datavalue.IOpaqueExpression>, IXmiElementReader<Auriga.Information.Datavalue.IOpaqueExpression>
    {
        ...
        public Auriga.Information.Datavalue.IOpaqueExpression Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                foreach (var token in (xmlReader.GetAttribute("bodies") ?? string.Empty).Split(WhitespaceSeparator, System.StringSplitOptions.RemoveEmptyEntries)) { poco.Bodies.Add(token); }
                ...
                foreach (var token in (xmlReader.GetAttribute("languages") ?? string.Empty).Split(WhitespaceSeparator, System.StringSplitOptions.RemoveEmptyEntries)) { poco.Languages.Add(token); }
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        break;
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of OpaqueExpression at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
