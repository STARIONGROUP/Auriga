...
namespace Auriga.Xmi.AutoGenXmiReaders.Sharedmodel
{
    ...
    public class SharedPkgReader : XmiElementReader<Auriga.Sharedmodel.ISharedPkg>, IXmiElementReader<Auriga.Sharedmodel.ISharedPkg>
    {
        ...
        public SharedPkgReader(IXmiElementCache cache, IXmiReaderFacade facade, ILoggerFactory loggerFactory)
        ...
        public Auriga.Sharedmodel.ISharedPkg Read(XmlReader xmlReader)
        {
            if (xmlReader == null)
            {
            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectSingleValueReference(poco, "OwnedGenericPkg", href); SkipElement(xmlReader); }
                            else { var contained = (Auriga.Sharedmodel.IGenericPkg)this.Facade.QueryElement(xmlReader); contained.Container = poco; poco.OwnedGenericPkg = contained; }
                            break;
                        }
                            ...
                            case "status":
                        {
                            ...
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of SharedPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
