            ...
            if (xmlReader.MoveToContent() == XmlNodeType.Element)
            {
                ...
                if (!xmlReader.IsEmptyElement)
                {
                    while (xmlReader.Read())
                    {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "CapellaElements", href); SkipElement(xmlReader); }
                            else { poco.CapellaElements.Add((Auriga.Capellacore.ICapellaElement)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            case "features":
                        {
                        ...
                        {
                            var href = xmlReader.GetAttribute("href");
                            if (!string.IsNullOrEmpty(href)) { CollectMultiValueReferences(poco, "SubGenericPkgs", href); SkipElement(xmlReader); }
                            else { poco.SubGenericPkgs.Add((Auriga.Sharedmodel.IGenericPkg)this.Facade.QueryElement(xmlReader)); }
                            break;
                        }
                            default:
                                this.Logger.LogTrace("Skipping unmapped element '{Element}' of GenericPkg at line {Line}:{Position}", xmlReader.LocalName, xmlLineInfo?.LineNumber ?? -1, xmlLineInfo?.LinePosition ?? -1);
